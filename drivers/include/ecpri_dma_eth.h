...
/**
 * struct ecpri_dma_eth_irq_comp_cb - ecpri_dma_eth client completion
 * callback notifier, ETH to invoke poll API on hdl to receive
 * completions (potentially multiple times to receive all)
 *
 ...
 */
typedef void (*ecpri_dma_eth_irq_comp_cb)(void *user_data,
					 ecpri_dma_eth_conn_hdl_t hdl);

/**
 * struct ecpri_dma_eth_tx_comp_cb - ecpri_dma_eth client tx completion
 * callback notifier
 *
 ...
 * @notify_tx_comp_irq: ecpri_dma_eth client tx completion callback notifier
 *                      when using Tx polling
 * @userdata_tx_irq:    userdata for ecpri_dma_eth tx irq cb
 */
struct ecpri_dma_eth_register_params {
	ecpri_dma_ready_cb notify_ready;
	void *userdata_ready;
	ecpri_dma_eth_irq_comp_cb notify_rx_comp;
	void *userdata_rx;
	ecpri_dma_eth_tx_comp_cb notify_tx_comp;
	void *userdata_tx;
	ecpri_dma_eth_irq_comp_cb notify_tx_comp_irq;
	void* userdata_tx_irq;
};
 ...
 *              not used when enable_tx_poll is set to true
...
int ecpri_dma_eth_rx_poll(ecpri_dma_eth_conn_hdl_t hdl, u32 budget,
			  ...
			  u32 *actual_num);
/**
 * ecpri_dma_eth_tx_mode_set() - ETH driver to set mode of Tx ring
 * @hdl:    [in] Unique connection handler generated by DMA driver to identify
 *          the Tx & Rx endpoints
 * @mode:   [in] New mode to set the Tx endpoint
 *
 * DMA will change the mode of the matching Tx endpoint according to the input
 * param.
 *
 * Returns:	0 on success, negative on failure
 */
...
/**
 * ecpri_dma_eth_tx_mode_get() - DMA driver to provide mode of Tx ring
 * @hdl:    [in] Unique connection handler generated by DMA driver to identify
 *          the Tx & Rx endpoints
 * @mode:   [out] Current mode to set the Tx endpoint
 *
 * DMA provide the mode of the matching Tx endpoint.
 *
 * Returns:	0 on success, negative on failure
 */
int ecpri_dma_eth_tx_mode_get(ecpri_dma_eth_conn_hdl_t hdl,
...
/**
 * ecpri_dma_eth_tx_poll() - ETH driver to poll tx completions on a specific
 * Tx endpoint with DMA driver
 * @hdl:		[in] Unique connection handler generated by DMA driver to
 *              identify the Tx & Rx endpoints
 * @budget:		[in] Number of completions to poll
 * @pkts:		[out] Array of completions recived on the specific Tx endpoint
 *              via the DMA.
 * @actual_num: [out] actual number of completions which were polled, may be
