...
static int ecpri_dma_mhi_client_test_suite_setup(void** ppriv)
{
	...
	int j;
	int ret = 0;
	int vms_num = ECPRI_DMA_MHI_CLIENT_FUNCTION_NUM;
	...
	if (ECPRI_DMA_GET_CTX_HW_VER() == ECPRI_HW_V1_0 &&
		ECPRI_DMA_GET_HW_FLAVOR() != ECPRI_HW_FLAVOR_DU_PCIE)
	{
		DMA_UT_ERR("MHI UTs require PCIe flavor\n");
		return -EINVAL;
	}
	else if (ECPRI_DMA_GET_CTX_HW_VER() == ECPRI_HW_V2_0 &&
		(ECPRI_DMA_GET_HW_FLAVOR() != ECPRI_HW_FLAVOR_DU_PCIE_5_X_6 &&
			ECPRI_DMA_GET_HW_FLAVOR() != ECPRI_HW_FLAVOR_DU_PCIE_4_X_9 &&
	...
	u32* expected_mask)
{
	...
	else
	{
		...
		else
		{
			...
			{
				switch (ECPRI_DMA_GET_HW_FLAVOR())
				{
	...
	enum ecpri_dma_ees ee, int gsi_id, bool redirect)
{
	...
	/* Ring CH DB */
	gsihal_write_reg_pnk(
		GSI_EE_n_GSI_CH_k_DOORBELL_0,
		gsi_id, ee, device_ch_idx,
		host_channels[host_ch_id].wp);
...
static int ecpri_dma_mhi_test_loopback_data_transfer(int idx,
	int host_src_ch_id, int host_dest_ch_id, enum ecpri_dma_ees ee, int gsi_id)
{
	u64 dest_orig_rp;
	u64 dest_new_rp;
	u64 src_orig_rp;
...
static int ecpri_dma_mhi_client_test_suite_mhi_init_all_vms(void* priv)
{
	...
	function =
		kmalloc(sizeof(struct mhi_dma_function_params) *
			ECPRI_DMA_MHI_CLIENT_FUNCTION_NUM, GFP_KERNEL);
	init_params =
		kmalloc(sizeof(struct mhi_dma_init_params) *
			ECPRI_DMA_MHI_CLIENT_FUNCTION_NUM, GFP_KERNEL);
	out_params =
		kmalloc(sizeof(struct mhi_dma_init_out) *
			ECPRI_DMA_MHI_CLIENT_FUNCTION_NUM, GFP_KERNEL);
	start_params =
		kmalloc(sizeof(struct mhi_dma_start_params) *
			ECPRI_DMA_MHI_CLIENT_FUNCTION_NUM, GFP_KERNEL);
...
static int ecpri_dma_mhi_client_test_suite_connect_endp_all(void* priv) {
	...
	/* Run for PF only */
	DMA_UT_DBG("Starting PF check Test_id %d\n", ECPRI_DMA_MHI_PF_ID);
 ...
 * is being accessed
 */
int ecpri_dma_mhi_client_test_suite_smmu_fault(void* priv)
{
	int ret = 0;
	int idx;
	struct mhi_dma_function_params function;
