/* da_secy_cfye_deverror.c
 *
 * Demo Application, MACsec transform test using SecY and CfyE API.
 */
...
/* Function to force error return at nth call to Device_Read/Write API functions
 */
void Device_ForceErrorAtNthCall(unsigned int n);

#define DA_MACSEC_NOF_DEVICES 2

/*----------------------------------------------------------------------------
 * da_secy_cfye_macsec_deverror
 */
bool
        ...
        bool fIngress)
{
...
#endif
    {
        ...
        {
            LOG_CRIT("DA_MACSEC: call failed as expected, CfyE_Device_Init, error=%d\n",
                     CfyE_Rc);
        }
        else
        {
            LOG_CRIT("DA_MACSEC: expected error return not detected: CfyE_Device_Init\n");
            return false;
        }
...

    {
        ...
        {
            LOG_CRIT("DA_MACSEC: call failed as expected, SecY_Device_Init, error=%d\n",
                     SecY_Rc);
        }
        else
        {
            return false;
            LOG_CRIT("DA_MACSEC: expected error return not detected: SecY_Device_Init\n");
        }
    ...
    /* Install default vPort for use with TCAM miss. */
    {
        ...
        if (CfyE_Rc != CFYE_STATUS_OK)
        {
            LOG_CRIT("DA_MACSEC: call failed as expected., CfyE_vPort_Add()=%d\n", CfyE_Rc);
            CfyE_vPort_Remove(CFYE_DEVICE_ID, CfyE_vPortHandle);
        }
        else
        {
            LOG_CRIT("DA_MACSEC: expected error return not detected: CfyE_vPort_add\n");
            goto error_exit;
        }
    ...
    /* Install drop SA with Drop-action is bypass-with-crc on default vPort */
    {
        ...
        if (SecY_Rc != SECY_STATUS_OK)
        {
            LOG_CRIT("DA_MACSEC: call failed as expected, SecY_SA_Add()=%d\n", SecY_Rc);
            SecY_SA_Remove(SECY_DEVICE_ID, SecY_SAHandleDrop);
        }
        else
        {
            LOG_CRIT("DA_MACSEC: expected error return not detected: SecY_SA_Add\n");
    ...
    /* Install SA with transform record */
    {
        ...
        Device_ForceErrorAtNthCall(3);
        SecY_Rc = SecY_SA_Add(SECY_DEVICE_ID, vPort, &SecY_SAHandle, &SA_Params);
        if (SecY_Rc != SECY_STATUS_OK)
        {
            LOG_CRIT("DA_MACSEC: call failed as expected, SecY_SA_Add()=%d\n", SecY_Rc);
            SecY_SA_Remove(SECY_DEVICE_ID, SecY_SAHandle);
        }
        else
        {
            LOG_CRIT("DA_MACSEC: expected error return not detected: SecY_SA_Add\n");
    ...
    /* Install Rule */
    {
        ...
        if (CfyE_Rc != CFYE_STATUS_OK)
        {
            LOG_CRIT("DA_MACSEC: call failed as expected, CfyE_Rule_Add()=%d\n", CfyE_Rc);
            CfyE_Rule_Remove(CFYE_DEVICE_ID, CfyE_RuleHandle);
        }
        else
        {
            LOG_CRIT("DA_MACSEC: expected error return not detected: CfyE_Rule_Add\n");
            goto error_exit;
        }
        ...
        if (CfyE_Rc != CFYE_STATUS_OK)
        {
            LOG_CRIT("DA_MACSEC: call Failed as expected, CfyE_Rule_Enable()=%d\n", CfyE_Rc);
        }
        else
        {
            LOG_CRIT("DA_MACSEC: expected error return not detected: CfyE_Rule_Enable\n");
            goto error_exit;
        }
    ...
    {
        unsigned int SAIndex,SAIndexDrop,RuleIndex;
        int rc;
        /* Convert handles to index, only index survives warmboot */
        SecY_SAIndex_Get(SecY_SAHandle, &SAIndex, NULL);
        SecY_SAIndex_Get(SecY_SAHandleDrop, &SAIndexDrop, NULL);
        CfyE_RuleIndex_Get(CfyE_RuleHandle, &RuleIndex);

        da_macsec_warmboot_shutdown();
        /* Restore the device areas. */
        rc =da_macsec_warmboot_restore(0,DA_MACSEC_NOF_DEVICES);
        if (rc) goto error_exit;
        /* Restore CfyE Area */
        Device_ForceErrorAtNthCall(2);
        rc = da_macsec_warmboot_restore(DA_MACSEC_NOF_DEVICES,1);
        if(rc)
        {
            Log_FormattedMessage("DA_MACSEC: WarmBoot_Restore (CfyE)) failed as expected\n");
        }
        else
        {
            LOG_CRIT("DA_MACSEC: expected error return not detected: CfyE WarmBoot_Restore\n");
            goto error_exit;
        }
        rc = da_macsec_warmboot_restore(DA_MACSEC_NOF_DEVICES,1);
        if (rc)
        {
            LOG_CRIT("DA_MACSEC: WarmBoot_Restore (CfyE) failed,\n");
            goto error_exit;
        }
        /* Restore SecY Area */
        Device_ForceErrorAtNthCall(2);
        rc = da_macsec_warmboot_restore(DA_MACSEC_NOF_DEVICES+1,1);
        if(rc)
        {
            Log_FormattedMessage("DA_MACSEC: WarmBoot_Restore (SecY)) failed as expected\n");
        }
        else
        {
            LOG_CRIT("DA_MACSEC: expected error return not detected: SecY WarmBoot_Restore\n");
            goto error_exit;
        }
        rc = da_macsec_warmboot_restore(DA_MACSEC_NOF_DEVICES+1,1);
        if (rc)
        {
            LOG_CRIT("DA_MACSEC: WarmBoot_Restore (SecY) failed,\n");
            goto error_exit;
        }

        /* Restore handles from index. */
        SecY_SAHandle_Get(SECY_DEVICE_ID, SAIndex, &SecY_SAHandle);
        SecY_SAHandle_Get(SECY_DEVICE_ID, SAIndexDrop, &SecY_SAHandleDrop);
        CfyE_RuleHandle_Get(CFYE_DEVICE_ID, RuleIndex, &CfyE_RuleHandle);
        CfyE_vPortHandle_Get(CFYE_DEVICE_ID, vPort, &CfyE_vPortHandle);
        CfyE_vPortHandle_Get(CFYE_DEVICE_ID, vPortDefault, &CfyE_vPortHandleDefault);
    }
#endif
        ...
        {
            {
                ...
                if (SecY_Rc != SECY_STATUS_OK)
                {
                    LOG_CRIT("DA_MACSEC: call failed as expected, SecY_SA_Statistics_I_Get()=%d\n",
                             SecY_Rc);
                }
                else
                {
                    LOG_CRIT("DA_MACSEC: expected error return not detected: SecY_SA_Statistics_I_Get\n");
                    goto error_exit;
                }
        ...
        {
            {
                ...
                if (SecY_Rc != SECY_STATUS_OK)
                {
                    LOG_CRIT("DA_MACSEC: call failed as expected, SecY_SA_Statistics_E_Get()=%d\n",
                             SecY_Rc);
                }
                else
                {
                    LOG_CRIT("DA_MACSEC: expected error return not detected: SecY_SA_Statistics_E_Getn");
                    goto error_exit;
...
error_exit:
    ...
    if (CfyE_Rc != CFYE_STATUS_OK)
    {
        LOG_CRIT("DA_MACSEC: Failed as epxected, CfyE_Rule_Disable()=%d\n", CfyE_Rc);
    }
    else
    {
        LOG_CRIT("DA_MACSEC: expected error return not detected: CfyE_Rule_Disable\n");
        fSuccess = false;
    }
    CfyE_Rc = CfyE_Rule_Disable(CFYE_DEVICE_ID, CfyE_RuleHandle, true);
    if (CfyE_Rc != CFYE_STATUS_OK)
    ...
    if (CfyE_Rc != CFYE_STATUS_OK)
    {
        LOG_CRIT("DA_MACSEC: call failed , CfyE_Rule_Remove()=%d\n", CfyE_Rc);
        fSuccess = false;
    }
    ...
    if (CfyE_Rc != CFYE_STATUS_OK)
    {
        LOG_CRIT("DA_MACSEC: call failed as expected, CfyE_vPort_Remove()=%d\n", CfyE_Rc);
    }
    else
    {
        LOG_CRIT("DA_MACSEC: expected error return not detected: CfyE_vPort_Remove\n");
        fSuccess = false;
    }
    ...
    if (SecY_Rc != SECY_STATUS_OK)
    {
        LOG_CRIT("DA_MACSEC: call failed as expected, SecY_SA_Remove()=%d\n", SecY_Rc);
    }
    else
    {
        LOG_CRIT("DA_MACSEC: expected error return not detected: SecY_SA_Remove\n");
        fSuccess = false;
    }
