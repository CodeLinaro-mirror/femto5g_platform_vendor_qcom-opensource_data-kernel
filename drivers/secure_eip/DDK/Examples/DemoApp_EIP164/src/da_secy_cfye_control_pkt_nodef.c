...
#define TEST_VLAN_ID 0x432  /* VLAN ID to be used in this test */
...
/* The test uses 3 vPorts
 *
 * 0 a vPort that carries all traffic on channel #0, will be must-secure.
 * 1 a vPort that carries all traffic on channel #1, will be should-secure.
 * 2 a vPort that carries all traffic on channel #1 with VLAN ID 0x432, will
 *   be should-secure and TCAM rule has higher priority than for vPort #1.
 *
 * 3 TCAM rules are used to match the traffic to the 3 vPorts. Control packets
 *   will match TCAM rules.
 */
...
static const uint8_t Control_Pkt_VLAN[] =
{
    0xd6, 0x09, 0xb1, 0xf0,
    0x56, 0x63, 0x7a, 0x0d,
    0x46, 0xdf, 0x99, 0x8d,
...
static const uint8_t Plaintext_Pkt_VLAN[] =
{
    0xd6, 0x09, 0xb1, 0xf0,
    0x56, 0x63, 0x7a, 0x0d,
    0x46, 0xdf, 0x99, 0x8d,
...
static const uint8_t MACsec_Pkt_VLAN[] =
{
    0xd6, 0x09, 0xb1, 0xf0,
    0x56, 0x63, 0x7a, 0x0d,
    0x46, 0xdf, 0x99, 0x8d,
...
static SecY_SAHandle_t SecY_SAHandleDummy[NR_VPORTS];
        ...
        bool fIngress)
{
    ...
    for (i=0; i<NR_VPORTS; i++)
    {
        ...
        SecY_SAHandleDummy[i] = SecY_SAHandle_NULL;
    ...
    /* Configure control packet and VLAN parsers */
    {
    ...
    for (i=0; i<NR_VPORTS; i++)
    {
        ...
        if (i==2)
            vPortParams.SecTagOffset = 16; /* vPort 2 has VLAN tag */
        else
    ...
    /* Install dummy SAs Install them before the TCAM rules, so no packets
       can be processed with them before they are disabled.*/
    for (i=0; i<NR_VPORTS; i++)
    {
        SecY_SA_t SA_Params;
        ...
        if (fIngress)
        {
            ...
            SA_Params.Params.Ingress.fAllowTagged = (i != 0);
        ...
        if (!fIngress)
        {
            ...
            SA_Params.Params.Egress.fAllowDataPkts = (i!=0);
            SA_Params.Params.Egress.PreSecTagAuthLength = 12;
        }
#endif /* DA_MACSEC_MODE_EGRESS */

        SecY_Rc = SecY_SA_Add(SECY_DEVICE_ID, vPortIndex[i], &SecY_SAHandleDummy[i], &SA_Params);
        if (SecY_Rc != SECY_STATUS_OK)
        {
        ...
        /* SecY_SA_Add() creates SA that is always enabled, disable it
           by calling SecY_SA_Update(). */
        SecY_Rc = SecY_SA_Update(SECY_DEVICE_ID, SecY_SAHandleDummy[i], &SA_Params);
        if (SecY_Rc != SECY_STATUS_OK)
        {
        ...
        if (fVerbose)
        {
            Log_FormattedMessage("DA_MACSEC: Dummy SA for vPort %d added\n",i);
        }
    }

    /* Create TCAM rules for vPorts */
    for (i=0; i<NR_VPORTS; i++)
    {
        ...
        if (i==2)
        {
            ...
            RuleParams.Data[2] = ((TEST_VLAN_ID >> 8) << 16) | ((TEST_VLAN_ID & 0xff) << 24);
        }

        CfyE_Rc = CfyE_Rule_Add_Index(CFYE_DEVICE_ID, CfyE_vPortHandle[i],
                                      &CfyE_RuleHandle[i], &RuleParams, NR_VPORTS-i);
        if (CfyE_Rc != CFYE_STATUS_OK)
        {
    ...
    /* At this point the Engine is ready to classify packets and send them
       to the respective vPorts, The actual MACsec SAs have not been installed
       yet. Control packets will always be passed and non-control packets
       are passd on should-secure channels, dropped on must-secure channels.*/
    if (fVerbose)
    {
    ...
    if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_SHOULD,
        ...
        goto error_exit;
    if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_SHOULD,
                         Plaintext_Pkt_VLAN, sizeof(Plaintext_Pkt_VLAN),
                         Plaintext_Pkt_VLAN, sizeof(Plaintext_Pkt_VLAN), false))
        goto error_exit;
    if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_MUST,
    ...
    if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_SHOULD,
        ...
        goto error_exit;
    if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_SHOULD,
                             Control_Pkt_VLAN, sizeof(Control_Pkt_VLAN),
                             Control_Pkt_VLAN, sizeof(Control_Pkt_VLAN), false))
        goto error_exit;

    /* Install MACsec SAs and remove dummy SAs */
    for (i=0; i<NR_VPORTS; i++)
    {
        SecY_SA_t SA_Params;
        ...
        SecY_Rc = SecY_SA_Add(SECY_DEVICE_ID, vPortIndex[i], &SecY_SAHandle[i], &SA_Params);
        if (SecY_Rc != SECY_STATUS_OK)
        {
            LOG_CRIT("DA_MACSEC: Failed, SecY_SA_Add()=%d\n", SecY_Rc);
            goto error_exit;
        ...
        /* Need to call SecY_SA_Update to get parameters updated */
        SecY_Rc = SecY_SA_Update(SECY_DEVICE_ID, SecY_SAHandle[i], &SA_Params);
        if (SecY_Rc != SECY_STATUS_OK)
        {
            LOG_CRIT("DA_MACSEC: Failed, SecY_SA_Update()=%d\n", SecY_Rc);
            goto error_exit;
        ...
        SecY_Rc = SecY_SA_Remove(SECY_DEVICE_ID, SecY_SAHandleDummy[i]);
        if (SecY_Rc != SECY_STATUS_OK)
        {
            LOG_CRIT("DA_MACSEC: Failed, SecY_SA_Remove()=%d\n", SecY_Rc);
            goto error_exit;
        }
        /* Mark the dummy SA as removed */
        SecY_SAHandleDummy[i] = SecY_SAHandle_NULL;
    }


    /* At this point, the MACsec SAs have been installed.
       ...
       Control packets are passed unchanged in all cases.
     */
    if (fIngress)
    {
        if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_MUST,
        ...
        if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_SHOULD,
            ...
            goto error_exit;
        if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_SHOULD,
                                 MACsec_Pkt_VLAN, sizeof(MACsec_Pkt_VLAN),
                                 Plaintext_Pkt_VLAN, sizeof(Plaintext_Pkt_VLAN), false))
            goto error_exit;
    ...
    else
    {
        ...
        if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_SHOULD,
                         Plaintext_Pkt_VLAN, sizeof(Plaintext_Pkt_VLAN),
                         MACsec_Pkt_VLAN, sizeof(MACsec_Pkt_VLAN), false))
            goto error_exit;
    }
    if (!da_secy_process_pkt(fIngress, fVerbose, CHANNEL_MUST,
                             Control_Pkt, sizeof(Control_Pkt),
    ...
    for (i=0; i<NR_VPORTS; i++)
    {
        ...
        /* Move current SA handles to dummy variables, so the situation will
           be the same as before installing MACsec SAs. */
        SecY_SAHandleDummy[i] = SecY_SAHandle[i];
        SecY_SAHandle[i] = SecY_SAHandle_NULL;

        if (fVerbose)
        {
            Log_FormattedMessage("DA_MACSEC: MACsec SA for vPort %d disabled\n",i);
        }
    }

    /* At this point, the MACsec SAs have been removed or disabled.
...
error_exit:
    ...
    for (i=0; i<NR_VPORTS; i++)
    {
        ...
        if(!SecY_SAHandle_IsSame(&SecY_SAHandleDummy[i], &SecY_SAHandle_NULL))
        {
            SecY_SA_Remove(SECY_DEVICE_ID, SecY_SAHandleDummy[i]);
            SecY_SAHandleDummy[i] = SecY_SAHandle_NULL;
        }
    }
