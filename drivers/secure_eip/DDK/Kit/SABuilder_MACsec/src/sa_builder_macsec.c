        ...
        SABuilder_AESCallback_t AES_CB)
{
    ...
    case SAB_OP_IPSEC:
        ...
        if (SAParams_p->direction == SAB_DIRECTION_EGRESS)
        {
            if ( (SAParams_p->flags & SAB_MACSEC_FLAG_LONGSEQ) != 0)
            {
                SABuffer_p[0] = SAB_CW0_IPSEC_EG64;
            }
            else
            {
                SABuffer_p[0] = SAB_CW0_IPSEC_EG32;
            }
            ...
            SABuffer_p[SAOffsets.SPIOffs] = SAParams_p->SPI;
        }
        else /* SAB_DIRECTION_INGRESS */
        {
            if ( (SAParams_p->flags & SAB_MACSEC_FLAG_LONGSEQ) != 0)
            {
                SABuffer_p[0] = SAB_CW0_IPSEC_IG64;
            }
            else
            {
                SABuffer_p[0] = SAB_CW0_IPSEC_IG32;
            }
            ...
            if ((SAParams_p->flags & SAB_IPSEC_FLAG_RETAIN_PADDING) != 0)
                SABuffer_p[0] |= SAB_CW0_KEEP_PAD;

            if ((SAParams_p->flags & SAB_IPSEC_FLAG_PAD_CHECK) != 0)
                SABuffer_p[0] |= (SAB_CW0_PAD_CHECK);

            if ((SAParams_p->flags & SAB_IPSEC_DISABLE_REPLAY_CHECK) != 0)
                SABuffer_p[0] &= ~(SAB_CW0_REPLAY_CHECK);

            if ((SAParams_p->flags & SAB_IPSEC_DISABLE_SA_TAG_INSERT) != 0)
                SABuffer_p[0] &= ~(SAB_CW0_IG_HDR_INSERT);
        }
        break;
#endif
...
#if defined(DRIVER_IPSEC)
        || SAParams_p->operation == SAB_OP_IPSEC
#endif
       )
    {
    ...
    {
        /* Fill in CtxSalt field. */
        if (SAOffsets.SaltOffs > 0 && NULL != SAParams_p->Salt_p)
        {
            SABuilderLib_CopyKeyMat(SABuffer_p,
                                    SAOffsets.SaltOffs,
                                    SAParams_p->Salt_p,
