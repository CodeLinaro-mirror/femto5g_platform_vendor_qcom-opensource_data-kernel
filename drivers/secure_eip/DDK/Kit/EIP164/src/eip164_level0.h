...
static inline int
EIP164_REPLAY_MASK_WR(
        Device_Handle_t Device,
        const unsigned int XformIndex,
        const uint32_t * ReplayMask_p)
{
    return EIP164_Write32Array(Device,
                               EIP164_REG_REPLAY_MASK(XformIndex),
                               ReplayMask_p,
                               4);
}

static inline int
EIP164_REPLAY_MASK_RD(
        Device_Handle_t Device,
        unsigned int const XformIndex,
        uint32_t * ReplayMask_p)
{
    return EIP164_Read32Array(Device,
                               EIP164_REG_REPLAY_MASK(XformIndex),
                               ReplayMask_p,
...
static inline int
EIP164_INGRESS_HDR_ETYPE_WR(
        Device_Handle_t Device,
        const uint16_t HdrEtype)
{
    return EIP164_Write32(Device,
                          EIP164_REG_PP_INGRESS_HDR_ETYPE,
                          (HdrEtype & MASK_16_BITS));
}
#endif
...
#if defined(EIP164_IPSEC_ENABLE)
static inline int
EIP164_SAM_FLOW_CTRL1_IPSEC_INGRESS_WR(
        Device_Handle_t Device,
        const unsigned int Index,
        const IPSEC_INGRESS_SAMFLOW_CTRL1_t * const SAMFlowCtrlVal)
{
    uint32_t RegVal = 0x2; /* Ingress */

    RegVal |= (uint32_t) ((((uint32_t) SAMFlowCtrlVal->DestPort) & MASK_2_BITS) << 2);

    if (SAMFlowCtrlVal->fFlowCryptAuth)
        RegVal |= BIT_5;
    else
        RegVal &= ~BIT_5;

    RegVal |= (uint32_t) ((((uint32_t) SAMFlowCtrlVal->DropAction) & MASK_2_BITS) << 6);

    if (SAMFlowCtrlVal->fReplayProtect)
        RegVal |= BIT_16;
    else
        RegVal &= ~BIT_16;

    if (SAMFlowCtrlVal->fConfProtect)
        RegVal |= BIT_17;
    else
        RegVal &= ~BIT_17;

    if (SAMFlowCtrlVal->fPadNotValidDrop)
        RegVal |= BIT_18;
    else
        RegVal &= ~BIT_18;

    if (SAMFlowCtrlVal->fPadLenFailDrop)
        RegVal |= BIT_19;
    else
        RegVal &= ~BIT_19;

    if (SAMFlowCtrlVal->fUpdateIP)
        RegVal |= BIT_20;
    else
        RegVal &= ~BIT_20;

    if (SAMFlowCtrlVal->fUpdateTTL)
        RegVal |= BIT_21;
    else
        RegVal &= ~BIT_21;
...
static inline int
EIP164_SAM_FLOW_CTRL1_IPSEC_EGRESS_WR(
        Device_Handle_t Device,
        const unsigned int Index,
        IPSEC_EGRESS_SAMFLOW_CTRL1_t * const SAMFlowCtrlVal)
{
    uint32_t RegVal = 0x3; /* egress*/
    ...
    SAMFlowCtrlVal->fProtectFrames = true;
    if (SAMFlowCtrlVal->fProtectFrames)
        RegVal |= BIT_16;
    else
        RegVal &= ~BIT_16;
    ...
    if (SAMFlowCtrlVal->fOuterIPHdr)
        RegVal |= BIT_18;
    else
        RegVal &= ~BIT_18;

    if (SAMFlowCtrlVal->fNAT_UDP)
        RegVal |= BIT_19;
    else
        RegVal &= ~BIT_19;
...
static inline int
EIP164_SAM_FLOW_CTRL1_IPSEC_INGRESS_RD(
        Device_Handle_t Device,
        const unsigned int Index,
        IPSEC_INGRESS_SAMFLOW_CTRL1_t * const SAMFlowCtrlVal)
{
    uint32_t RegVal = 0;
    int rc = 0;
    ...
    if (!rc)
    {
        SAMFlowCtrlVal->DestPort = (RegVal >> 2) & MASK_2_BITS;
        SAMFlowCtrlVal->fFlowCryptAuth = (RegVal & BIT_5) != 0;
        SAMFlowCtrlVal->DropAction = (RegVal >> 6) & MASK_2_BITS;
        SAMFlowCtrlVal->fReplayProtect = (RegVal & BIT_16) != 0;
        SAMFlowCtrlVal->fConfProtect = (RegVal & BIT_17) != 0;
        SAMFlowCtrlVal->fPadNotValidDrop = (RegVal & BIT_18) != 0;
        SAMFlowCtrlVal->fPadLenFailDrop = (RegVal & BIT_19) != 0;
        SAMFlowCtrlVal->fUpdateIP = (RegVal & BIT_20) != 0;
        SAMFlowCtrlVal->fUpdateTTL = (RegVal & BIT_21) != 0;
    }
...
static inline int
EIP164_SAM_FLOW_CTRL1_IPSEC_EGRESS_RD(
        Device_Handle_t Device,
        const unsigned int Index,
        IPSEC_EGRESS_SAMFLOW_CTRL1_t * const SAMFlowCtrlVal)
{
    ...
    if (!rc)
    {
        ...
        SAMFlowCtrlVal->fFlowCryptAuth = (RegVal & BIT_5) != 0;
        SAMFlowCtrlVal->DropAction = (RegVal >> 6) & MASK_2_BITS;
        SAMFlowCtrlVal->fProtectFrames = (RegVal & BIT_16) != 0;
        SAMFlowCtrlVal->fConfProtect = (RegVal & BIT_17) != 0;
        SAMFlowCtrlVal->fOuterIPHdr = (RegVal & BIT_18) != 0;
        SAMFlowCtrlVal->fNAT_UDP = (RegVal & BIT_19) != 0;
        SAMFlowCtrlVal->fUpdateIP = (RegVal & BIT_20) != 0;
        SAMFlowCtrlVal->fUpdateUDP = (RegVal & BIT_21) != 0;
    }
        ...
        )
{
    ...
    {
#if defined(EIP164_IPSEC_ENABLE)
        *fBypassStatus_p  = ((RegVal & BIT_31) != 0);
        *fIPsec_p         = ((RegVal & BIT_30) != 0);
#endif
