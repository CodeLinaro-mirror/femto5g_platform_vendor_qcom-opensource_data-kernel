...
#if defined(EIP164_IPSEC_ENABLE)
        , const EIP164_ChannelMode_t ChannelMode
#endif
        )
{
    Device_Handle_t Device;
...
#if defined(EIP164_IPSEC_ENABLE)
    if (EIP164_CHANNEL_MODE_MACSEC == ChannelMode)
#endif
    {
...
#if defined(EIP164_IPSEC_ENABLE)
    else if (EIP164_CHANNEL_MODE_IPSEC == ChannelMode)
    {
        switch (SA_p->ActionType)
        {
        case EIP164_SECY_SA_ACTION_IPSEC_INGRESS:
        {
            IPSEC_INGRESS_SAMFLOW_CTRL1_t SAMFlowCtrlVal;

            SAMFlowCtrlVal.DestPort         = SA_p->DestPort;
            SAMFlowCtrlVal.fFlowCryptAuth   = false;
            SAMFlowCtrlVal.DropAction       = SA_p->DropType;
            SAMFlowCtrlVal.fReplayProtect   = SA_p->Params.IPsecIngress.fReplayProtect;
            SAMFlowCtrlVal.fConfProtect     = SA_p->Params.IPsecIngress.fConfProtect;
            SAMFlowCtrlVal.fPadNotValidDrop = SA_p->Params.IPsecIngress.fPadNotValidDrop;
            SAMFlowCtrlVal.fPadLenFailDrop  = SA_p->Params.IPsecIngress.fPadLenFailDrop;
            SAMFlowCtrlVal.fUpdateIP  = SA_p->Params.IPsecIngress.fUpdateIP;
            SAMFlowCtrlVal.fUpdateTTL = SA_p->Params.IPsecIngress.fUpdateTTL;

            /* Enable IPsec SA */
            rc = EIP164_SAM_FLOW_CTRL1_IPSEC_INGRESS_WR(Device,
                                                        vPort,
                                                        &SAMFlowCtrlVal);
        ...
        case EIP164_SECY_SA_ACTION_IPSEC_EGRESS:
        {
            IPSEC_EGRESS_SAMFLOW_CTRL1_t SAMFlowCtrlVal;

            SAMFlowCtrlVal.DestPort       = SA_p->DestPort;
            SAMFlowCtrlVal.fFlowCryptAuth = false;
            SAMFlowCtrlVal.DropAction     = SA_p->DropType;
            SAMFlowCtrlVal.fProtectFrames = SA_p->Params.IPsecEgress.fProtectFrames;
            SAMFlowCtrlVal.fConfProtect   = SA_p->Params.IPsecEgress.fConfProtect;
            SAMFlowCtrlVal.fOuterIPHdr    = SA_p->Params.IPsecEgress.fOuterIPHdr;
            SAMFlowCtrlVal.fNAT_UDP   =   SA_p->Params.IPsecEgress.fNAT_UDP;
            SAMFlowCtrlVal.fUpdateIP  =   SA_p->Params.IPsecEgress.fUpdateIP;
            SAMFlowCtrlVal.fUpdateUDP =   SA_p->Params.IPsecEgress.fUpdateUDP;

            rc = EIP164_SAM_FLOW_CTRL1_IPSEC_EGRESS_WR(Device,
                                                       vPort,
                                                       &SAMFlowCtrlVal);
            if (rc) return rc;
        }
    ...
    else if (EIP164_CHANNEL_MODE_IPSEC == ChannelMode)
    {
        ...
        case EIP164_SECY_SA_ACTION_IPSEC_INGRESS:
        {
            ...
            /* Check if the SA type is supported in the device mode */
             ...
             rc = EIP164_SAM_FLOW_CTRL1_IPSEC_INGRESS_RD(Device,
                                                         vPort,
                                                         &SAMFlowCtrlVal);
             if (rc) return rc;

             SA_p->Params.IPsecIngress.fReplayProtect = SAMFlowCtrlVal.fReplayProtect;
             SA_p->Params.IPsecIngress.fConfProtect = SAMFlowCtrlVal.fConfProtect;
             SA_p->Params.IPsecIngress.fPadNotValidDrop = SAMFlowCtrlVal.fPadNotValidDrop;
             SA_p->Params.IPsecIngress.fPadLenFailDrop = SAMFlowCtrlVal.fPadLenFailDrop;
             SA_p->Params.IPsecIngress.fUpdateIP = SAMFlowCtrlVal.fUpdateIP;
        }
        break;
        ...
        case EIP164_SECY_SA_ACTION_IPSEC_EGRESS:
        {
            ...
            /* Check if the SA type is supported in the device mode */
             if (TrueIOArea_p->Mode == EIP164_MODE_INGRESS)
             {
              ...
              rc = EIP164_SAM_FLOW_CTRL1_IPSEC_EGRESS_RD(Device,
                                                         vPort,
                                                         &SAMFlowCtrlVal);
              if (rc) return rc;

            SA_p->Params.IPsecEgress.fProtectFrames = SAMFlowCtrlVal.fProtectFrames;
            SA_p->Params.IPsecEgress.fConfProtect = SAMFlowCtrlVal.fConfProtect;
            SA_p->Params.IPsecEgress.fOuterIPHdr = SAMFlowCtrlVal.fOuterIPHdr;
            SA_p->Params.IPsecEgress.fNAT_UDP = SAMFlowCtrlVal.fNAT_UDP;
            SA_p->Params.IPsecEgress.fUpdateIP = SAMFlowCtrlVal.fUpdateIP;
            SA_p->Params.IPsecEgress.fUpdateUDP = SAMFlowCtrlVal.fUpdateUDP;
        }
        break;
...
#if defined(EIP164_IPSEC_ENABLE)
/*----------------------------------------------------------------------------
 * EIP164_SecY_SA_ReplayMask_Write
 */
EIP164_Error_t
EIP164_SecY_SA_ReplayMask_Write(
        const EIP164_IOArea_t * const IOArea_p,
        const unsigned int SAIndex)
{
    ...
    /* Write replay mask */
    return EIP164_REPLAY_MASK_WR(Device,
                                 SAIndex,
                                 ReplayMask);
}

EIP164_Error_t
EIP164_SecY_SA_ReplayMask_Read(
        const EIP164_IOArea_t * const IOArea_p,
        const unsigned int SAIndex,
        uint32_t * const ReplayMask_p)
{
    Device_Handle_t Device;
    const volatile EIP164_True_IOArea_t * const TrueIOArea_p = IOAREA_IN(IOArea_p);
    ...
    /* Write replay mask */
    return EIP164_REPLAY_MASK_RD(Device,
                                 SAIndex,
                                 ReplayMask_p);
}
#endif
...
EIP164_SecY_Stat_IPsec_E_Get(
        const EIP164_IOArea_t * const IOArea_p,
        const unsigned int SecY_Index,
        EIP164_SecY_Statistics_IPsec_E_t * const Stats_p,
        const bool fClear)
{
    Device_Handle_t Device;
    const volatile EIP164_True_IOArea_t * const TrueIOArea_p = IOAREA_IN(IOArea_p);
...
#if defined(EIP164_IPSEC_ENABLE)
/*----------------------------------------------------------------------------
 * EIP164_SecY_Stat_IPsec_I_Get
 */
EIP164_Error_t
EIP164_SecY_Stat_IPsec_I_Get(
        const EIP164_IOArea_t * const IOArea_p,
        const unsigned int SecY_Index,
        EIP164_SecY_Statistics_IPsec_I_t * const Stats_p,
        const bool fClear)
{
    Device_Handle_t Device;
    const volatile EIP164_True_IOArea_t * const TrueIOArea_p = IOAREA_IN(IOArea_p);
    ...
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SECY_STATISTICS(SecY_Index),
                              EIP164_SECY_STAT_I_IN_PKTS_LATE_HDR,
                              &Stats_p->InPktsLateHdr.low,
                              &Stats_p->InPktsLateHdr.hi,
                              fClear);
    if (rc) return rc;

    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SECY_STATISTICS(SecY_Index),
                              EIP164_SECY_STAT_I_IN_PKTS_IP_MISMATCH,
                              &Stats_p->InPktsIPMismatch.low,
                              &Stats_p->InPktsIPMismatch.hi,
                              fClear);
    if (rc) return rc;
...
EIP164_SecY_SA_Stat_IPsec_I_Get(
        const EIP164_IOArea_t * const IOArea_p,
        const unsigned int SAIndex,
        EIP164_SecY_SA_Stat_IPsec_I_t * const Stat_p,
        const bool fClear)
{
    Device_Handle_t Device;
    const volatile EIP164_True_IOArea_t * const TrueIOArea_p = IOAREA_IN(IOArea_p);
    ...
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SA_STATISTICS(SAIndex),
                              EIP164_SA_IPSEC_STAT_I_IN_PKTS_PAD_NOT_VALID,
                              &Stat_p->InPktsPadNotValid.low,
                              &Stat_p->InPktsPadNotValid.hi,
                              fClear);
    if (rc) return rc;
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SA_STATISTICS(SAIndex),
                              EIP164_SA_IPSEC_STAT_I_IN_PKTS_REPLAYED,
                              &Stat_p->InPktsReplayed.low,
                              &Stat_p->InPktsReplayed.hi,
                              fClear);
    if (rc) return rc;
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SA_STATISTICS(SAIndex),
    ...
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SA_STATISTICS(SAIndex),
                              EIP164_SA_IPSEC_STAT_I_IN_PKTS_PAD_LEN_FAIL,
                              &Stat_p->InPktsPadLenFail.low,
                              &Stat_p->InPktsPadLenFail.hi,
                              fClear);
    if (rc) return rc;
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SA_STATISTICS(SAIndex),
    ...
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SA_STATISTICS(SAIndex),
                              EIP164_SA_IPSEC_STAT_I_IN_PKTS_PAD_DUMMY,
                              &Stat_p->InPktsPadDummy.low,
                              &Stat_p->InPktsPadDummy.hi,
                              fClear);
    if (rc) return rc;
