...
#if defined(EIP164_IPSEC_ENABLE)
        *fPad_NH_Check_p    = (RegVal & BIT_12) != 0;
        *fPadLenCheck_p     = (RegVal & BIT_13) != 0;
        *fPadPatternCheck_p = (RegVal & BIT_14) != 0;
        *fPadDummyCheck_p   = (RegVal & BIT_15) != 0;
#endif
    }
        ...
        bool * const    fPad_NH_Check_p)
{
    ...
    rc = EIP66_Read32(Device, EIP66_REG_STAT(Channel), &RegVal);

    if (!rc)
    {
        *fPad_NH_Check_p    = (RegVal & BIT_19) != 0;
        *fPadLenCheck_p     = (RegVal & BIT_18) != 0;
        *fPadPatternCheck_p = (RegVal & BIT_17) != 0;
        *fPadDummyStatus_p  = (RegVal & BIT_16) != 0;
    }
