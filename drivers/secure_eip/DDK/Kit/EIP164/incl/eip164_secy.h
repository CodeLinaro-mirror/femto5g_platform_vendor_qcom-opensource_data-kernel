...
typedef struct
{
    ...
    EIP164_UI64_t OutPktsSANotInUse;
} EIP164_SecY_Statistics_IPsec_E_t;
...
/* SecY IPsec Ingress Statistics */
typedef struct
{
    /* Packet counters */
    EIP164_UI64_t TransformErrorPkts;

    /* Counts packets that are late at SOP */
    EIP164_UI64_t InPktsLateHdr;

    /* Counts packet that have mismatch of outer IP header and inner IP header
     * type
     */
    EIP164_UI64_t InPktsIPMismatch;

    /** Packet counters - Not Using SA */
    EIP164_UI64_t InPktsSANotInUse;
} EIP164_SecY_Statistics_IPsec_I_t;
...
typedef union
{
    ...
    EIP164_SecY_Statistics_IPsec_E_t IPsecEgress;
    EIP164_SecY_Statistics_IPsec_I_t IPsecIngress;
...
typedef enum
{
    ...
    EIP164_SECY_SA_ACTION_IPSEC_INGRESS,
    EIP164_SECY_SA_ACTION_IPSEC_EGRESS,
#endif
} EIP164_SecY_SA_ActionType_t;
...
typedef struct
{
    ...
    /** true - When nextPN reaching all 1s, it rolls over to value 1 without a
      * sequence number error generated.
      */
    bool fRollOverMode;
    ...
    /**
      ...
      * 01b - for 32-bit sequence number.
...
typedef struct
{
    ...
    /** true - Enable late packet detection at SOP */
    bool fLateHdrCheck;
...
#if defined(EIP164_IPSEC_ENABLE)
        EIP164_SecY_SA_IPsec_E_t IPsecEgress;
        EIP164_SecY_SA_IPsec_I_t IPsecIngress;
#endif
    } Params;
...
typedef struct
{
    ...
    /* Packet counters */
    EIP164_UI64_t InPktsPadNotValid;
    EIP164_UI64_t InPktsReplayed;
    EIP164_UI64_t InPktsLate;
    EIP164_UI64_t InPktsOK;

    EIP164_UI64_t InPktsPadLenFail;
    EIP164_UI64_t InPktsNotValid;
    EIP164_UI64_t InPktsNotUsingSA;
    EIP164_UI64_t InPktsPadDummy;
} EIP164_SecY_SA_Stat_IPsec_I_t;
...
typedef union
{
    ...
    EIP164_SecY_SA_Stat_IPsec_I_t IPsecIngress;
...
typedef struct
{
    ...
    bool fPktNumThrMode;
    uint16_t MACsecEtherType;
    bool fPadNextHdrCheck;
    bool fPadLenCheck;
    bool fPadPatternCheck;
    bool fPadDummyCheck;
} EIP164_ChannelContext_t;
...
typedef struct
{
    ...
    EIP164_Channel_Rule_SecTAG_t RuleSecTAG;
 ...
 *
 * fBypassStatus_p (output)
 *      Bypass status on the given channel.
 *
 * fIPsec_p (output)
 *      IPsec status on the given channel.
 ...
 *
 * fIPsecEnable (input)
 *      Enable IPsec processing on the given channel.
 *
 * Return value
 *      EIP164_NO_ERROR : Operation is completed.
 *      EIP164_ARGUMENT_ERROR : Passed wrong argument.
 ...
 *
 *
 * fPad_NH_Check_p (output)
 *      Mode for IPsec pad Next Header field check
 *              True  - Enable IPsec pad Next Header field check.
 *              False - Disable IPsec pad Next Header field check.
 *
 * fPadLenCheck_p (output)
 *      Mode for IPsec pad length field check.
 *              True -  Enable IPsec pad length field check.
 *              false - Disable IPsec pad length field check.
 *
 * fPadPatternCheck_p (output)
 *      Mode for IPsec pad pattern field check
 *              True  - Enable IPsec pad pattern field check.
 *              False - Disable IPsec pad pattern field check.
 *
 * fPadDummyCheck_p (output)
 *      Mode for Enable IPsec dummy packet detection
 *              True  - Enable IPsec dummy packet detection.
 *              False - Disable IPsec dummy packet detection.
 *
 * Return value
 *      EIP164_NO_ERROR : Operation is completed.
 *      EIP164_ARGUMENT_ERROR : Passed wrong argument.
...
#if defined(EIP164_IPSEC_ENABLE)
        , const bool fPad_NH_Check,
        const bool fPadLenCheck,
        const bool fPadPatternCheck,
        const bool fPadDummyCheck
#endif
        );
 ...
 * fPad_NH_Check_p (output)
 *      Mode for IPsec pad Next Header field check
 *              True  - IPsec pad Next Header field check enabled.
 *              False - IPsec pad Next Header field check disabled.
 *
 * fPadLenCheck_p (output)
 *      Mode for IPsec pad length field check.
 *              True -  IPsec pad length field check enabled.
 *              false - IPsec pad length field check disabled.
 *
 * fPadPatternCheck_p (output)
 *      Mode for IPsec pad pattern field check
 *              True  - IPsec pad pattern field check enabled.
 *              False - IPsec pad pattern field check disabled.
 *
 * fPadDummyCheck_p (output)
 *      Mode for Enable IPsec dummy packet detection
 *              True  - IPsec dummy packet detection enabled.
 *              False - IPsec dummy packet detection disabled.
 *
 * Return value
 *      EIP164_NO_ERROR : Operation is completed.
 *      EIP164_ARGUMENT_ERROR : Passed wrong argument.
...
#if defined(EIP164_IPSEC_ENABLE)
        , bool * const fPad_NH_Check_p,
        bool * const fPadLenCheck_p,
        bool * const fPadPatternCheck_p,
        bool * const fPadDummyCheck_p
#endif
        );
 ...
 *
 * ChannelMode (input)
 *      MACsec or IPsec Flow mode of the channel
 *
 * This function is NOT re-entrant for the same Device.
 * This function is re-entrant for different Devices.
 *
 ...
 *
 * Writes the Replay Mask record as extension to the transform record.
 *
 * API use order:
 *      This function may be called after the EIP164_SecY_SA_Write() function
 *      for the same SAIndex of the same Device.
 ...
 *
 * Reads the Replay Mask record as extension to the transform record.
 *
 * API use order:
 *      This function may be called after the EIP164_SecY_SA_Write() function
 *      for the same SAIndex of the same Device.
 ...
 *
 * Stat_p (output)
 *      Pointer to a memory location where the IPsec SA statistics will be stored.
 *
 * fClear (input)
 *      Use write action to clear statistics registers
 *
