...
#include "api_secy_ext.h"  /* Extended SecY API */
        ...
        SecY_SA_t * const SA_p)
{
    ...
    else if (DeviceSA_p->ActionType == EIP164_SECY_SA_ACTION_IPSEC_EGRESS)
    {
        SA_p->ActionType = SECY_SA_ACTION_IPSEC_EGRESS;

        SA_p->Params.IPsecEgress.fSAInUse       =
            DeviceSA_p->Params.IPsecEgress.fSAInUse;
    ...
    else if (DeviceSA_p->ActionType == EIP164_SECY_SA_ACTION_IPSEC_INGRESS)
    {
        SA_p->ActionType = SECY_SA_ACTION_IPSEC_INGRESS;

        SA_p->Params.IPsecIngress.fSAInUse =
            DeviceSA_p->Params.IPsecIngress.fSAInUse;
        ...
        SecY_Role_t * const Role_p)
{
    ...
    *Role_p = SecY_Device[DeviceId]->Role;
        ...
        SecY_SA_t * const SA_p)
{
    ...
    memset(SA_p, 0, sizeof(SecY_SA_t));
    Rc = EIP164_SecY_SAMFlow_Read(&SecY_Device[DeviceId]->IOArea,
                                  vPortId,
                                  &DeviceSA
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
                                  , SecY_Device[DeviceId]->vPortDscr_p[vPortId].ChannelMode
#endif
                                 );
        ...
        uint8_t * const SCI_p)
{
...
#ifdef ADAPTER_EIP164_MODE_INGRESS
    {
        ...
        if (SCI_p)
        {
            ...
            memcpy(SCI_p, SecY_Device[DeviceId]->SCDscr_p[SCIndex].SCI, 8);
        }
...
#ifdef ADAPTER_EIP164_MODE_INGRESS
    {
        unsigned int ActiveSAIndex[4] = {0};
        bool fSAInUse[4] = {false, false, false, false};
        ...
        if (fSAInUse[0] && ActiveSAIndex[0] == SAIndex)
        {
            SA_p->Params.IPsecIngress.fSAInUse = true;
        }
        else if (fSAInUse[1] && ActiveSAIndex[1] == SAIndex)
        {
            SA_p->Params.IPsecIngress.fSAInUse = true;
...
#ifdef ADAPTER_EIP164_MODE_EGRESS
        {
            unsigned int ActiveSAIndex = 0;
            bool fSAInUse = false;

            SA_p->SA_WordCount = 20;
            Rc = EIP164_SecY_SA_Installation_Verify(&SecY_Device[DeviceId]->IOArea,
                                                    vPort,
                                                    &ActiveSAIndex,
                                                    &fSAInUse);
        ...
        SecY_SAHandle_t * const NextSAHandle_p)
{
    ...
    for ( ; SAIndex < SecY_Device[DeviceId]->SACount; SAIndex++)
    {
        if (SecY_Device[DeviceId]->SADscr_p[SAIndex].Magic ==
        ...
        uint32_t * const SCIndex_p)
{
    ...
    SCList = SecY_Device[DeviceId]->vPortDscr_p[vPort].SCList;
    if (*TmpHandle_p == NULL)
    {
    ...
    *TmpHandle_p = cur;
    if (cur != NULL)
    {
        ...
        memcpy(NextSCI_p, SCDscr_p->SCI, 8);
        if (SCIndex_p)
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
        Log_FormattedMessage("IPsec=%d ", ChannelParams.fIPsec);
#endif
        ...
        const bool fIncludeSA)
{
    ...
    else
    {
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
        case SECY_SA_ACTION_IPSEC_INGRESS:
            Log_FormattedMessage("Action is IPsec INGRESS\n");
            Log_FormattedMessage(
                "fRetainPad = %d fPadCheck = %d fReplayCheck = %d"
                "fIgHdrInsert = %d fReplayProtect = %d fConfProtect = %d"
                "fPadNotValidDrop = %d fPadLenF a i lDrop = %d  fUpdateIP = %d"
                "fUpdateTTL = %d\n",
                SAParams.Params.IPsecIngress.fRetainPad,
                SAParams.Params.IPsecIngress.fPadCheck,
                SAParams.Params.IPsecIngress.fReplayCheck,
                SAParams.Params.IPsecIngress.fIgHdrInsert,
        ...
        case SECY_SA_ACTION_IPSEC_EGRESS:
            Log_FormattedMessage("Action is IPsec EGRESS\n");
            Log_FormattedMessage(
                "fRollOverMode = %d\n"
                "CryptoAlg = %d fReplayCheck = %d\n"
                "fIgHdrInsert = %d fUpdateUDP = %d fUpdateIP = %d\n"
                "fNAT_UDP = %d fOuterIPHdr = %d fConfProtect = %d\n"
                "fProtectFrames = %d\n",
                SAParams.Params.IPsecEgress.fRollOverMode,
                SAParams.Params.IPsecEgress.CryptoAlg,
                SAParams.Params.IPsecEgress.fReplayCheck,
                SAParams.Params.IPsecEgress.fIgHdrInsert,
        ...
        if (Role==SECY_ROLE_EGRESS)
        {
            ...
            case SECY_SA_ACTION_IPSEC_EGRESS:
            {
                SecY_SecY_Stat_IPsec_E_t SecY_Stat;

                SecY_Rc = SecY_SecY_Statistics_IPsec_E_Get(DeviceId,
                                                           vPortId,
                                                           &SecY_Stat,
                                                           false);
                if (SecY_Rc != SECY_STATUS_OK)
                {
                    LOG_CRIT("SecY_SA_Statistics_IPsec_E_Get returned error %d\n",
                             SecY_Rc);
                    return SecY_Rc;
                }
                ...
                Log_FormattedMessage(
                    ...
                    SecY_Stat.OutPktsTransformError.Lo,
                    SecY_Stat.OutPktsTransformError.Hi,
                    SecY_Stat.OutPktsSANotInUse.Lo,
                    SecY_Stat.OutPktsSANotInUse.Hi,
                    SecY_Stat.OutPktsUntagged.Lo,
                    SecY_Stat.OutPktsUntagged.Hi);
            }
            break;
        ...
        if (Role==SECY_ROLE_INGRESS)
        {
            ...
            switch (SAParams.ActionType)
            {
                ...
                case SECY_SA_ACTION_IPSEC_INGRESS:
                {
                    SecY_SecY_Stat_IPsec_I_t SecY_Stat;

                    SecY_Rc = SecY_SecY_Statistics_IPsec_I_Get(DeviceId,
                                                               vPortId,
                                                               &SecY_Stat,
                                                               false);
                    ...
                    Log_FormattedMessage(
                        "SecY Stats InPktsTransformErr         lo=%10u hi=%10u\n"
                        "           InPktsLateHdr              lo=%10u hi=%10u\n"
                        "           InPktsIPMismatch           lo=%10u hi=%10u\n"
                        "           InPktsSANotInUse           lo=%10u hi=%10u\n",
                        SecY_Stat.InPktsTransformError.Lo,
                        SecY_Stat.InPktsTransformError.Hi,
                        SecY_Stat.InPktsLateHdr.Lo,
                        SecY_Stat.InPktsLateHdr.Hi,
                        SecY_Stat.InPktsIPMismatch.Lo,
                        SecY_Stat.InPktsIPMismatch.Hi,
                        SecY_Stat.InPktsSANotInUse.Lo,
                        SecY_Stat.InPktsSANotInUse.Hi);
                }
                break;
#endif /* ADAPTER_EIP164_IPSEC_ENABLE */
            };
        ...
        const bool fAllSAs)
{
    ...
    else
    {
        ...
        if (Role == SECY_ROLE_INGRESS)
        {
            SecY_Channel_Mode_t ChannelMode = SECY_MODE_MACSEC;
            unsigned int SAIndex;
            unsigned int vPort;
            ...
            ChannelMode = SecY_Device[DeviceId]->vPortDscr_p[vPort].ChannelMode;
#endif

            if (SECY_MODE_MACSEC == ChannelMode)
            {
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
            else if (SECY_MODE_IPSEC == ChannelMode)
            {
                SecY_SA_Stat_IPsec_I_t SA_Stat;

                SecY_Rc = SecY_SA_Statistics_IPsec_I_Get(DeviceId,
                                                         SAHandle,
                                                         &SA_Stat,
                                                         false);
                if (SecY_Rc != SECY_STATUS_OK)
                ...
                Log_FormattedMessage(
                    "SA Stats InPktsPadNotValid            lo=%10u hi=%10u\n"
                    "         InPktsReplayed               lo=%10u hi=%10u\n"
                    "         InPktsLate                   lo=%10u hi=%10u\n"
                    "         InPktsOK                     lo=%10u hi=%10u\n"
                    "         InPktsPadLenF a i l          lo=%10u hi=%10u\n"
                    "         InPktsNotValid               lo=%10u hi=%10u\n"
                    "         InPktsNotUsingSA             lo=%10u hi=%10u\n"
                    "         InPktsPadDummy               lo=%10u hi=%10u\n"
                    "         InOctetsDecrypted            lo=%10u hi=%10u\n"
                    "         InOctetsValidated            lo=%10u hi=%10u\n",
                    SA_Stat.InPktsPadNotValid.Lo,
                    SA_Stat.InPktsPadNotValid.Hi,
                    SA_Stat.InPktsReplayed.Lo,
                    SA_Stat.InPktsReplayed.Hi,
                    SA_Stat.InPktsLate.Lo,
                    SA_Stat.InPktsLate.Hi,
                    SA_Stat.InPktsOK.Lo,
                    SA_Stat.InPktsOK.Hi,
                    SA_Stat.InPktsPadLenFail.Lo,
                    SA_Stat.InPktsPadLenFail.Hi,
                    SA_Stat.InPktsNotValid.Lo,
                    SA_Stat.InPktsNotValid.Hi,
                    SA_Stat.InPktsNotUsingSA.Lo,
                    SA_Stat.InPktsNotUsingSA.Hi,
                    SA_Stat.InPktsPadDummy.Lo,
                    SA_Stat.InPktsPadDummy.Hi,
                    SA_Stat.InOctetsDecrypted.Lo,
                    SA_Stat.InOctetsDecrypted.Hi,
                    SA_Stat.InOctetsValidated.Lo,
                    SA_Stat.InOctetsValidated.Hi);
