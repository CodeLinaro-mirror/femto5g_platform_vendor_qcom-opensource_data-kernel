        ...
        const SecY_ChannelConf_t * const ChannelConfig_p)
{
    ...
    if (ChannelConfig_p->Params.ChannelCount)
    {
        ...
        for (i = 0; i < ChannelCount; i++)
        {
            ...
            Rc = EIP164_Rules_SecTag_Update(&SecY_Device[DeviceId]->IOArea,
                                            ch_p->ChannelId,
                                            &Dev_SecTag_Rules);
            if (Rc != EIP164_NO_ERROR)
                return SECY_DLIB_ERROR(Rc);
        ...
        SecY_Channel_t * const ChannelConfig_p)
{
    ...
    /* Enable or disable low latency bypass */
    Rc = EIP164_Device_ChannelControl_Read(&SecY_Device[DeviceId]->IOArea,
                                           ChannelId,
                                           ...
                                           ,&ChannelConfig_p->fBypassStatus,
                                           &ChannelConfig_p->fIPsec
#endif
                                           );
    ...
    Rc = EIP164_Rules_SecTag_Read(&SecY_Device[DeviceId]->IOArea,
                                  ChannelId,
                                  &Dev_SecTag_Rules);
    if (Rc != EIP164_NO_ERROR)
    {
    ...
    Rc = EIP164_Device_Channel_CtxControl_Read(&SecY_Device[DeviceId]->IOArea,
                                               ChannelId,
                                               &ChannelConfig_p->fPktNumThrMode,
                                               &ChannelConfig_p->EtherType
#ifdef ADAPTER_EIP164_IPSEC_ENABLE
                                               ,&ChannelConfig_p->fPad_NH_Check,
                                               &ChannelConfig_p->fPadLenCheck,
                                               &ChannelConfig_p->fPadPatternCheck,
                                               &ChannelConfig_p->fPadDummyCheck
#endif
                                               );
    if (Rc != EIP164_NO_ERROR)
    {
        ...
        const SecY_Settings_t * const Settings_p)
{
    ...
    memcpy(&Settings.EOPTimeoutCtrl, &Settings_p->EOPTimeoutCtrl, sizeof(SecY_Ch_Mask_t));

#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
    Settings.IngressHdrEtype = Settings_p->IngressHdrEtype;
#endif
    ...
    Settings.VPInvDropType       = SECY_SA_DROP_INTERNAL;
    Settings.FLInvDropType       = SECY_SA_DROP_INTERNAL;
    ...
    Rc = EIP164_Device_Init(&SecY_Device[DeviceId]->IOArea, Device, &Settings);
    ...
    if (Rc == EIP164_BUSY_RETRY_LATER)
    {
        ...
        do
        {
            ...
            Rc = EIP164_Device_Is_Init_Done(&SecY_Device[DeviceId]->IOArea);
            if (Rc != EIP164_BUSY_RETRY_LATER)
            {
                break;
            }
    ...
    Rc = EIP164_Device_Clocks_Off(&SecY_Device[DeviceId]->IOArea,
    ...
    Rc = EIP164_Device_Clocks_Off(&SecY_Device[DeviceId]->IOArea, 0);
    if (Rc != EIP164_NO_ERROR)
    {
        Adapter_Free(SecY_Device[DeviceId]);
        SecY_Device[DeviceId] = NULL;
    ...
    Rc = EIP164_Device_Clocks_On(&SecY_Device[DeviceId]->IOArea,
                             EIP164_SECY_ALL_CLOCKS,
                             1);
    ...
    SecY_Device[DeviceId]->SACount      = Cp.EIP164_Options.SA_Count;
    SecY_Device[DeviceId]->SCCount      = Cp.EIP164_Options.SC_Count;
    SecY_Device[DeviceId]->ChannelCount = Cp.EIP164_Options.Channel_Count;
    SecY_Device[DeviceId]->vPortCount   = Cp.EIP164_Options.vPort_Count;

    /* Restrict any counts to the values specified by Settings_p */
    if (Settings_p->MaxChannelCount > 0 &&
        Settings_p->MaxChannelCount < SecY_Device[DeviceId]->ChannelCount)
    {
        LOG_CRIT("%s: restricting ChannelCount to %u\n",
                 __func__,Settings_p->MaxChannelCount);
        SecY_Device[DeviceId]->ChannelCount = Settings_p->MaxChannelCount;
    }
    if (Settings_p->MaxvPortCount > 0 &&
        Settings_p->MaxvPortCount < SecY_Device[DeviceId]->vPortCount)
    {
        LOG_CRIT("%s: restricting vPortCount to %u\n",
                 __func__,Settings_p->MaxvPortCount);
        SecY_Device[DeviceId]->vPortCount = Settings_p->MaxvPortCount;
    }
    if (Settings_p->MaxSCCount > 0 &&
        Settings_p->MaxSCCount < SecY_Device[DeviceId]->SCCount)
    {
        LOG_CRIT("%s: restricting SCCount to %u\n",
                 __func__,Settings_p->MaxSCCount);
        SecY_Device[DeviceId]->SCCount = Settings_p->MaxSCCount;
    }
    if (Settings_p->MaxSACount > 0 &&
        Settings_p->MaxSACount < SecY_Device[DeviceId]->SACount)
    {
        LOG_CRIT("%s: restricting SACount to %u\n",
                 __func__,Settings_p->MaxSACount);
        SecY_Device[DeviceId]->SACount = Settings_p->MaxSACount;
    }
...
#ifdef ADAPTER_EIP164_IPSEC_ENABLE
            SecY_Device[DeviceId]->vPortDscr_p[i].ChannelMode = SECY_MODE_MACSEC;
#endif
        ...
        const unsigned int DeviceId)
{
    ...
    Rc = EIP164_Device_Uninit(&SecY_Device[DeviceId]->IOArea);
    if (Rc != EIP164_NO_ERROR && Rc != EIP164_BUSY_RETRY_LATER)
    {
        ...
        unsigned int * const MaxSCCount_p)
{
    ...
    if (MaxChannelCount_p != NULL)
        *MaxChannelCount_p = SecY_Device[DeviceId]->ChannelCount;

    if (MaxvPortCount_p != NULL)
        *MaxvPortCount_p = SecY_Device[DeviceId]->vPortCount;

    if (MaxSACount_p != NULL)
        *MaxSACount_p = SecY_Device[DeviceId]->SACount;

    if (MaxSCCount_p != NULL)
        *MaxSCCount_p = SecY_Device[DeviceId]->SCCount;
        ...
        const SecY_Device_Params_t * const Device_p)
{
    ...
    if (Device_p->StatControl_p)
    {
        ...
        EIP164_Rc = EIP164_StatisticsControl_Update(&SecY_Device[DeviceId]->IOArea,
                                                    &StatControl);
        ...
        if (EIP164_Rc == EIP164_BUSY_RETRY_LATER)
        {
            ...
            do
            {
                ...
                EIP164_Rc =
                        EIP164_Device_Is_Init_Done(&SecY_Device[DeviceId]->IOArea);
                if (EIP164_Rc != EIP164_BUSY_RETRY_LATER)
                {
                    break;
                }
        ...
        if (SecY_Device[DeviceId]->Role == SECY_ROLE_INGRESS)
        {
        ...
        EIP164_Rc = EIP164_Device_SA_Statistics_Enable(&SecY_Device[DeviceId]->IOArea,
                SACntrMask *
                (!(StatCtrl_p->CountIncDisCtrl & SECY_SA_COUNT_INC_DIS)));
        if (EIP164_Rc != EIP164_NO_ERROR)
        {
        ...
        EIP164_Rc =
            EIP164_Device_IFC_Statistics_Enable(&SecY_Device[DeviceId]->IOArea,
                IFCCntrMask *
                (!(StatCtrl_p->CountIncDisCtrl & SECY_IFC_COUNT_INC_DIS)));
        if (EIP164_Rc != EIP164_NO_ERROR)
        {
        ...
        EIP164_Rc =
            EIP164_Device_IFC1_Statistics_Enable(&SecY_Device[DeviceId]->IOArea,
                IFCCntrMask *
                (!(StatCtrl_p->CountIncDisCtrl & SECY_IFC1_COUNT_INC_DIS)));
        if (EIP164_Rc != EIP164_NO_ERROR)
        {
        ...
        EIP164_Rc =
            EIP164_Device_SecY_Statistics_Enable(&SecY_Device[DeviceId]->IOArea,
        ...
        EIP164_Rc =
            EIP164_Device_RxCAM_Statistics_Enable(&SecY_Device[DeviceId]->IOArea,
                RxCAMCntrMask *
                (!(StatCtrl_p->CountIncDisCtrl & SECY_RXCAM_COUNT_INC_DIS)));
        if (EIP164_Rc != EIP164_NO_ERROR)
        {
    ...
    if (Device_p->EOPConf_p)
    {
        ...
        Rc = EIP164_Device_EOP_Configure(&SecY_Device[DeviceId]->IOArea,
    ...
    if (Device_p->ECCConf_p)
    {
        ...
        Rc = EIP164_Device_ECC_Configure(&SecY_Device[DeviceId]->IOArea,
    ...
    {
        Rc = EIP164_Device_SA_Retire_Write(&SecY_Device[DeviceId]->IOArea,
        ...
        SecY_Device_Params_t * const Control_p)
{
    ...
    if (StatControl_p)
    {
        ...
        Rc = EIP164_StatisticsControl_Read(&SecY_Device[DeviceId]->IOArea,
                                           &StatControl);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        Rc = EIP164_Device_SA_Statistics_Enabled_Get(&SecY_Device[DeviceId]->IOArea,
                                           &Mask);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        Rc = EIP164_Device_SecY_Statistics_Enabled_Get(&SecY_Device[DeviceId]->IOArea,
                                           &Mask);
        if (Rc != EIP164_NO_ERROR)
        {
            LOG_CRIT("%s: Failed to read statistics enabled EIP-164 device for device %d "
        ...
        Rc = EIP164_Device_IFC_Statistics_Enabled_Get(&SecY_Device[DeviceId]->IOArea,
                                           &Mask);
        if (Rc != EIP164_NO_ERROR)
        {
            LOG_CRIT("%s: Failed to read statistics enabled EIP-164 device for device %d "
        ...
        Rc = EIP164_Device_IFC1_Statistics_Enabled_Get(&SecY_Device[DeviceId]->IOArea,
                                           &Mask);
        if (Rc != EIP164_NO_ERROR)
        {
            LOG_CRIT("%s: Failed to read statistics enabled EIP-164 device for device %d "
        ...
        Rc = EIP164_Device_RxCAM_Statistics_Enabled_Get(&SecY_Device[DeviceId]->IOArea,
                                           &Mask);
        if (Rc != EIP164_NO_ERROR)
        {
            LOG_CRIT("%s: Failed to read statistics enabled EIP-164 device for device %d "
    ...
    if (ChConf_p)
    {
        ...
        ChConf_p->Params.ChannelCount = SecY_Device[DeviceId]->ChannelCount;
    ...
    if (EOPConf_p)
    {
        Rc = EIP164_Device_EOP_Configuration_Get(&SecY_Device[DeviceId]->IOArea,
    ...
    if (ECCConf_p)
    {
        Rc = EIP164_Device_ECC_Configuration_Get(&SecY_Device[DeviceId]->IOArea,
    ...
    {
        Rc = EIP164_Device_SA_Retire_Read(&SecY_Device[DeviceId]->IOArea,
        ...
        SecY_DeviceStatus_t * const DeviceStatus_p)
{
    ...
    if (ECCStatus_p)
    {
        Rc = EIP164_Device_ECCStatus_Get(&SecY_Device[DeviceId]->IOArea,
                                         ECCStatus);
        if (Rc != EIP164_NO_ERROR)
        {
    ...
    {
        Rc = EIP164_Device_PktProcessDebug_Get(&SecY_Device[DeviceId]->IOArea,
                                               &PktProcessDebug);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        const SecY_Ch_Mask_t * const ChannelMask_p)
{
    ...
    Rc = EIP164_Device_InsertSOP(&SecY_Device[DeviceId]->IOArea,
        ...
        const SecY_Ch_Mask_t * const ChannelMask_p)
{
    ...
    Rc = EIP164_Device_InsertEOP(&SecY_Device[DeviceId]->IOArea,
                                 (const EIP164_Ch_Mask_t * const) ChannelMask_p);
    if (Rc != EIP164_NO_ERROR)
    {
        ...
        const unsigned int BypassLength)
{
    ...
    Rc = EIP164_SecY_CryptAuth_BypassLen_Update(&SecY_Device[DeviceId]->IOArea,
                                                BypassLength);
    if (Rc != EIP164_NO_ERROR)
    {
        ...
        unsigned int * const BypassLength_p)
{
    ...
    Rc = EIP164_SecY_CryptAuth_BypassLen_Get(&SecY_Device[DeviceId]->IOArea,
                                             BypassLength_p);
    if (Rc != EIP164_NO_ERROR)
    {
        ...
        const SecY_Channel_Rule_SecTAG_t * const SecTag_Rules_p)
{
    ...
    if (ChannelId >= SecY_Device[DeviceId]->ChannelCount)
        return SECY_ERROR_BAD_PARAMETER;
...
#ifdef ADAPTER_EIP164_IPSEC_ENABLE
    IDENTIFIER_NOT_USED(fBypassStatus);
    IDENTIFIER_NOT_USED(fIPsec);
#endif
        ...
        const bool fBypass)
{
    ...
    if (fBypass)
    {
        ...
        Rc = EIP164_Device_ChannelControl_Write(&SecY_Device[DeviceId]->IOArea,
                                                ChannelId,
                                                0,     /* disable latency */
