...
#include "adapter_secy_warmboot.h" /* SecY WarmBoot functions. */
...
/* Number of bytes for the WarmBoot Channel Mode bitmap */
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
#define SECY_WARMBOOT_CH_MODE_BYTE_COUNT(n) (((n) + 7) / 8)
#endif
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
                      + SECY_WARMBOOT_CH_MODE_BYTE_COUNT(Cp.EIP164_Options.vPort_Count)
#endif
                      ;
    ...
    unsigned int DeviceId)
{
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
    SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap =
        Adapter_Alloc(SECY_WARMBOOT_CH_MODE_BYTE_COUNT(SecY_Device[DeviceId]->vPortCount));
    if (SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap == NULL)
    {
        LOG_CRIT("%s: Failed to allocate WarmBoot Channel Mode bitmap. "
                 "for device %d\n",
                 __func__, DeviceId);
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
                           + SECY_WARMBOOT_CH_MODE_BYTE_COUNT(SecY_Device[DeviceId]->vPortCount)
#endif
                           , &SecY_Device[DeviceId]->WarmBoot_AreaId))
    {
        ...
        Adapter_Free(SecY_Device[DeviceId]->WarmBoot_SA_Bitmap);
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
        Adapter_Free(SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap);
#endif
    ...
    memset(SecY_Device[DeviceId]->WarmBoot_SA_Bitmap,
           0,
           SECY_WARMBOOT_SA_BYTE_COUNT(SecY_Device[DeviceId]->SACount));

#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
    memset(SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap,
           0,
           SECY_WARMBOOT_CH_MODE_BYTE_COUNT(SecY_Device[DeviceId]->vPortCount));
#endif
 ...
 *
 * Mark Channel Mode as IPsec.
 */

void SecYLib_WarmBoot_ChMode_Set(
    unsigned int DeviceId,
    unsigned int vPortIndex)
{
    ...
    SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap[ByteIndex] |= BitMask;

    WarmBootLib_Write(SecY_Device[DeviceId]->WarmBoot_AreaId,
                      SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap + ByteIndex,
 ...
 *
 * Mark Channel Mode as MACsec.
 */
void SecYLib_WarmBoot_ChMode_Clear(
    unsigned int DeviceId,
    unsigned int vPortIndex)
{
    unsigned int ByteIndex = vPortIndex / 8;
...
/*----------------------------------------------------------------------------
 * SecYLib_WarmBoot_ChMode_Get
 *
 * Check if the Channel Mode is IPsec
 */
static inline bool
SecYLib_WarmBoot_ChMode_Get(
    unsigned int DeviceId,
    unsigned int vPortIndex)
{
    ...
    return (SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap[ByteIndex] & BitMask) != 0;
}
#endif
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
    Adapter_Free(SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap);
    SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap = NULL;
#endif
        ...
        unsigned int AreaId)
{
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
    SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap =
        Adapter_Alloc(SECY_WARMBOOT_CH_MODE_BYTE_COUNT(vPortCount));
    if (SecY_Device[DeviceId]->WarmBoot_ChMode_Bitmap == NULL)
    {
        LOG_CRIT("%s: Failed to allocate WarmBoot Channel Mode bitmap. "
                 "for device %d\n",
    ...
    WarmBootLib_Read(AreaId,
                     ...
                     SECY_WARMBOOT_BASE_BYTE_COUNT +
                     SECY_WARMBOOT_SA_BYTE_COUNT(SACount),
                     SECY_WARMBOOT_CH_MODE_BYTE_COUNT(vPortCount));
#endif
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
            if (SecYLib_WarmBoot_ChMode_Get(DeviceId, i) == 0)
            {
                SecY_Device[DeviceId]->vPortDscr_p[i].ChannelMode = SECY_MODE_MACSEC;
            }
            else
            {
                SecY_Device[DeviceId]->vPortDscr_p[i].ChannelMode = SECY_MODE_IPSEC;
            }
#endif
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
                        if (SecYLib_WarmBoot_ChMode_Get(DeviceId, vPort) == 0)
                        {
                            SecY_Device[DeviceId]->vPortDscr_p[vPort].ChannelMode = SECY_MODE_MACSEC;
                        }
                        else
                        {
                            SecY_Device[DeviceId]->vPortDscr_p[vPort].ChannelMode = SECY_MODE_IPSEC;
                        }
#endif
                    }
                    else
