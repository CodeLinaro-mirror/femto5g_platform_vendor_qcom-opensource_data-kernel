        ...
        const unsigned int DeviceId)
{
    ...
    Rc = EIP164_Device_Sync(&SecY_Device[DeviceId]->IOArea);
    ...
    if (Rc == EIP164_BUSY_RETRY_LATER)
    {
        ...
        while (LoopCounter)
        {
            ...
            Rc = EIP164_Device_Is_Sync_Done(&SecY_Device[DeviceId]->IOArea);
...
SecYLib_Device_Lock(
        const unsigned int DeviceId)
{
    Adapter_Lock_Acquire(&SecY_Device_StaticFields[DeviceId].Lock,
                         &SecY_Device_StaticFields[DeviceId].Flags);
}
...
SecYLib_Device_Unlock(
        const unsigned int DeviceId)
{
    Adapter_Lock_Release(&SecY_Device_StaticFields[DeviceId].Lock,
                         &SecY_Device_StaticFields[DeviceId].Flags);
}
        ...
        uint32_t * Transform_p)
{
    ...
    Rc = EIP164_SecY_SA_Read(&SecY_Device[DeviceId]->IOArea,
                             SAIndex,
                             WordOffset,
                             WordCount,
                             Transform_p);
        ...
        unsigned int vPort)
{
    ...
    Role = SecY_Device[DeviceId]->Role;
#ifdef ADAPTER_EIP164_MODE_EGRESS
    if (Role == SECY_ROLE_EGRESS || Role == SECY_ROLE_EGRESS_INGRESS)
    {
        Rc = EIP164_SecY_Stat_E_Clear(&SecY_Device[DeviceId]->IOArea, vPort);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        Rc = EIP164_SecY_Ifc_Stat_E_Clear(&SecY_Device[DeviceId]->IOArea,
                                          vPort);
        if (Rc != EIP164_NO_ERROR)
        {
    ...
    {
        Rc = EIP164_SecY_Stat_I_Clear(&SecY_Device[DeviceId]->IOArea, vPort);
        if (Rc != EIP164_NO_ERROR)
        {
            LOG_CRIT("%s: Failed to clear EIP164 SecY statistics for device %d "
                     "(%s), error %d\n",
        ...
        Rc = EIP164_SecY_Ifc_Stat_I_Clear(&SecY_Device[DeviceId]->IOArea,
                                          vPort);
        if (Rc != EIP164_NO_ERROR)
        {
            LOG_CRIT("%s: Failed to clear EIP164 Ifc statistics for device %d "
        ...
        const SecY_Notify_t * const Notify_p)
{
    ...
    if (Notify_p->ChannelId >= SecY_Device[DeviceId]->ChannelCount)
        return SECY_ERROR_BAD_PARAMETER;
#endif
