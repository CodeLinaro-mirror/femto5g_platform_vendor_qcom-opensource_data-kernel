        ...
        const bool fSync)
{
    ...
    Rc = EIP164_SecY_SA_Stat_E_Get(&SecY_Device[DeviceId]->IOArea,
                                   SAIndex,
                                   &DeviceStat,
        ...
        const bool fSync)
{
    ...
    Rc = EIP164_SecY_SA_Stat_I_Get(&SecY_Device[DeviceId]->IOArea,
                                   SAIndex,
                                   &DeviceStat,
                                   ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
        ...
        const bool fSync)
{
    ...
    SecY_Status_t SecY_Rc;
    unsigned int SAIndex;
    EIP164_SecY_SA_Stat_IPsec_I_t DeviceStat;
    ...
    LOG_INFO("\n\t EIP164_SecY_SA_Stat_IPsec_I_Get \n");

    /* Read this Ingress SA statistics from the device */
    Rc = EIP164_SecY_SA_Stat_IPsec_I_Get(&SecY_Device[DeviceId]->IOArea,
                                         SAIndex,
                                         &DeviceStat,
                                         ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
    ...
    SecYLib_DeviceCounter_Copy(&DeviceStat.InPktsPadNotValid,
                               &Stat_p->InPktsPadNotValid);
    SecYLib_DeviceCounter_Copy(&DeviceStat.InPktsReplayed,
                               &Stat_p->InPktsReplayed);
    SecYLib_DeviceCounter_Copy(&DeviceStat.InPktsLate,
                               &Stat_p->InPktsLate);
    SecYLib_DeviceCounter_Copy(&DeviceStat.InPktsOK,
                               &Stat_p->InPktsOK);
    SecYLib_DeviceCounter_Copy(&DeviceStat.InPktsPadLenFail,
                               &Stat_p->InPktsPadLenFail);
    SecYLib_DeviceCounter_Copy(&DeviceStat.InPktsNotValid,
                               &Stat_p->InPktsNotValid);
    SecYLib_DeviceCounter_Copy(&DeviceStat.InPktsNotUsingSA,
                               &Stat_p->InPktsNotUsingSA);
    SecYLib_DeviceCounter_Copy(&DeviceStat.InPktsPadDummy,
                               &Stat_p->InPktsPadDummy);
        ...
        unsigned int * const NumSecYIndexes_p)
{
    ...
    *NumSecYIndexes_p = 0;
    ...
    MaxIndex = SecY_Device[DeviceId]->vPortCount;
    MaxSummaryIndex = (SecY_Device[DeviceId]->vPortCount + 31) / 32;
    ...
    for (i = 1; i <= MaxSummaryIndex; i++)
    {
        ...
        Summary = 0;
        Rc = EIP164_Device_CountSummary_PSecY_Read(&SecY_Device[DeviceId]->IOArea,
                                                  i,
                                                  &Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        if (Summary != 0)
        {
            ...
            /* Clear it after read */
            Rc = EIP164_Device_CountSummary_PSecY_Clear(&SecY_Device[DeviceId]->IOArea,
                                                        i,
                                                        Summary);
            if (Rc != EIP164_NO_ERROR)
            {
        ...
        uint32_t * const CountSummarySecY_p)
{
    ...
    Rc = EIP164_Device_CountSummary_SecY_Read(&SecY_Device[DeviceId]->IOArea,
                                              SecYIndex,
                                              &Summary);
    if (Rc != EIP164_NO_ERROR)
    {
    ...
    if (Summary != 0)
    {
        ...
        /* Clear the bits set */
        Rc = EIP164_Device_CountSummary_SecY_Clear(&SecY_Device[DeviceId]->IOArea,
                                                   SecYIndex,
                                                   Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        unsigned int * const NumIfcIndexes_p)
{
    ...
    *NumIfcIndexes_p = 0;
    ...
    for (i = 1; i <= MaxSummaryIndex; i++)
    {
        ...
        /* Read summary register */
        Summary = 0;
        Rc = EIP164_Device_CountSummary_PIFC_Read(&SecY_Device[DeviceId]->IOArea,
                                                  i,
                                                  &Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        if (Summary != 0)
        {
            ...
            /* Clear it after read */
            Rc = EIP164_Device_CountSummary_PIFC_Clear(&SecY_Device[DeviceId]->IOArea,
                                                       i,
                                                       Summary);
            if (Rc != EIP164_NO_ERROR)
            {
        ...
        uint32_t * const CountSummaryIfc_p)
{
    ...
    /* Read IFC1 count summary register */
    Rc = EIP164_Device_CountSummary_IFC_Read(&SecY_Device[DeviceId]->IOArea,
                                             IfcIndex,
                                             &Summary);
    if (Rc != EIP164_NO_ERROR)
    {
    ...
    if (Summary != 0)
    {
        ...
        /* Clear the bits set */
        Rc = EIP164_Device_CountSummary_IFC_Clear(&SecY_Device[DeviceId]->IOArea,
                                                  IfcIndex,
                                                  Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        unsigned int * const NumIfcIndexes_p)
{
    ...
    *NumIfcIndexes_p = 0;
    ...
    for (i = 1; i <= MaxSummaryIndex; i++)
    {
        ...
        /* Read summary register */
        Summary = 0;
        Rc = EIP164_Device_CountSummary_PIFC1_Read(&SecY_Device[DeviceId]->IOArea,
                                                   i,
                                                   &Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        if (Summary != 0)
        {
            ...
            /* Clear it after read */
            Rc = EIP164_Device_CountSummary_PIFC1_Clear(&SecY_Device[DeviceId]->IOArea,
                                                        i,
                                                        Summary);
            if (Rc != EIP164_NO_ERROR)
            {
        ...
        uint32_t * const CountSummaryIfc_p)
{
    ...
    /* Read IFC1 count summary register */
    Rc = EIP164_Device_CountSummary_IFC1_Read(&SecY_Device[DeviceId]->IOArea,
                                              IfcIndex,
                                              &Summary);
    if (Rc != EIP164_NO_ERROR)
    {
    ...
    if (Summary != 0)
    {
        ...
        /* Clear the bits set */
        Rc = EIP164_Device_CountSummary_IFC1_Clear(&SecY_Device[DeviceId]->IOArea,
                                                   IfcIndex,
                                                   Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        unsigned int * const NumRxCAMIndexes_p)
{
    ...
    *NumRxCAMIndexes_p = 0;
    ...
    MaxIndex = SecY_Device[DeviceId]->SCCount;
    MaxSummaryIndex = (SecY_Device[DeviceId]->SCCount + 31) / 32;
    ...
    for (i = 1; i <= MaxSummaryIndex; i++)
    {
        ...
        /* Read summary register */
        Summary = 0;
        Rc = EIP164_Device_CountSummary_PRxCAM_Read(&SecY_Device[DeviceId]->IOArea,
                                                    i,
                                                    &Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        if (Summary != 0)
        {
            ...
            /* Clear it after read */
            Rc = EIP164_Device_CountSummary_PRxCAM_Clear(&SecY_Device[DeviceId]->IOArea,
                                                         i,
                                                         Summary);
            if (Rc != EIP164_NO_ERROR)
            {
        ...
        unsigned int * const NumSAIndexes_p)
{
    ...
    *NumSAIndexes_p = 0;
    ...
    MaxIndex = SecY_Device[DeviceId]->SACount;
    MaxSummaryIndex = (SecY_Device[DeviceId]->SACount + 31) / 32;
    ...
    for (i = 1; i <= MaxSummaryIndex; i++)
    {
        ...
        /* Read summary register */
        Summary = 0;
        Rc = EIP164_Device_CountSummary_PSA_Read(&SecY_Device[DeviceId]->IOArea,
                                                 i,
                                                 &Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        if (Summary != 0)
        {
            ...
            /* Clear it after read */
            Rc = EIP164_Device_CountSummary_PSA_Clear(&SecY_Device[DeviceId]->IOArea,
                                                      i,
                                                      Summary);
            if (Rc != EIP164_NO_ERROR)
            {
        ...
        uint32_t * const CountSummarySA_p)
{
    ...
    Rc = EIP164_Device_CountSummary_SA_Read(&SecY_Device[DeviceId]->IOArea,
                                            SAIndex,
                                            &Summary);
    if (Rc != EIP164_NO_ERROR)
    {
    ...
    if (Summary != 0)
    {
        ...
        /* Clear the bits set */
        Rc = EIP164_Device_CountSummary_SA_Clear(&SecY_Device[DeviceId]->IOArea,
                                                 SAIndex,
                                                 Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        const bool fSync)
{
    ...
    Rc = EIP164_SecY_Stat_E_Get(&SecY_Device[DeviceId]->IOArea, vPort, &Stats,
                                   ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
    {
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
/*----------------------------------------------------------------------------
 * SecY_SA_Statistics_IPsec_E_Get
 */
SecY_Status_t
        ...
        const bool fSync)
{
    EIP164_Error_t Rc;
    SecY_Status_t SecY_Rc;
    EIP164_SecY_Statistics_IPsec_E_t Stats;
    ...
    LOG_INFO("\n\t EIP164_SecY_Stat_IPsec_E_Get \n");

    Rc = EIP164_SecY_Stat_IPsec_E_Get(&SecY_Device[DeviceId]->IOArea, vPort, &Stats,
                                   ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
    {
        LOG_CRIT("%s: failed for device %d, error %d\n",
        ...
        const bool fSync)
{
    ...
    Rc = EIP164_SecY_Stat_I_Get(&SecY_Device[DeviceId]->IOArea, vPort, &Stats,
                                   ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
    {
        LOG_CRIT("%s: failed for device %d, error %d\n",
    ...
    SecYLib_DeviceCounter_Copy(&Stats.InPktsSANotInUse,
                               &Stats_p->InPktsSANotInUse);
        ...
        const bool fSync)
{
    EIP164_Error_t Rc;
    SecY_Status_t SecY_Rc;
    EIP164_SecY_Statistics_IPsec_I_t Stats;
    ...
    Rc = EIP164_SecY_Stat_IPsec_I_Get(&SecY_Device[DeviceId]->IOArea,
                                      vPort,
                                      &Stats,
                                      ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
    ...
    SecYLib_DeviceCounter_Copy(&Stats.InPktsLateHdr,
                               &Stats_p->InPktsLateHdr);

    SecYLib_DeviceCounter_Copy(&Stats.InPktsIPMismatch,
                               &Stats_p->InPktsIPMismatch);
        ...
        const bool fSync)
{
    ...
    Rc = EIP164_SecY_Ifc_Stat_E_Get(&SecY_Device[DeviceId]->IOArea,
                                    vPort,
                                    &Stats,
                                   ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
        ...
        const bool fSync)
{
    ...
    Rc = EIP164_SecY_Ifc_Stat_I_Get(&SecY_Device[DeviceId]->IOArea,
                                    vPort,
                                    &Stats,
                                   ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
        ...
        const bool fSync)
{
    ...
    Rc = EIP164_SecY_RxCAM_Stat_Get(&SecY_Device[DeviceId]->IOArea,
                                    SCIndex,
                                    &Stats,
                                    ADAPTER_EIP164_WRITE_TO_DECREMENT);
    if (Rc != EIP164_NO_ERROR)
        ...
        unsigned int * const NumSAIndexes_p)
{
    ...
    *NumSAIndexes_p = 0;
    ...
    for (i = 1; i <= MaxSummaryIndex; i++)
    {
        ...
        /* Read summary register */
        Rc = EIP164_Device_SAPnThrSummary_Read(&SecY_Device[DeviceId]->IOArea,
                                               i,
                                               &Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        if (Summary != 0)
        {
            ...
            /* Clear it after read */
            Rc = EIP164_Device_SAPnThrSummary_Clear(&SecY_Device[DeviceId]->IOArea,
                                                    i,
                                                    Summary);
            if (Rc != EIP164_NO_ERROR)
            {
        ...
        unsigned int * const NumSAIndexes_p)
{
    ...
    *NumSAIndexes_p = 0;
    ...
    for (i = 1; i <= MaxSummaryIndex; i++)
    {
        ...
        /* Read summary register */
        Rc = EIP164_Device_SAExpiredSummary_Read(&SecY_Device[DeviceId]->IOArea,
                                                 i,
                                                 &Summary);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        if (Summary != 0)
        {
            ...
            /* Clear it after read */
            Rc = EIP164_Device_SAExpiredSummary_Clear(&SecY_Device[DeviceId]->IOArea,
                                                      i,
                                                      Summary);
            if (Rc != EIP164_NO_ERROR)
            {
