...
#include "c_adapter_eip164.h"             /* ADAPTER_EIP164_DRIVER_NAME */

/* Adapter internal SecY API. */
#include "adapter_secy_support.h"  /* Adapter internal SecY API */
...
#include "clib.h"               /* ZEROINIT(),memcpy(),memcmp() */
        ...
        EIP164_SecY_SA_t * const DeviceSA_p)
{
    ...
    else if (SA_p->ActionType == SECY_SA_ACTION_IPSEC_EGRESS)
    {
        DeviceSA_p->ActionType = EIP164_SECY_SA_ACTION_IPSEC_EGRESS;
        DeviceSA_p->Params.IPsecEgress.fRollOverMode  =
            SA_p->Params.IPsecEgress.fRollOverMode;
        DeviceSA_p->Params.IPsecEgress.fEncrAuth      =
            SA_p->Params.IPsecEgress.fEncrAuth;
        DeviceSA_p->Params.IPsecEgress.CryptoAlg      =
            SA_p->Params.IPsecEgress.CryptoAlg;
        DeviceSA_p->Params.IPsecEgress.fReplayCheck   =
            SA_p->Params.IPsecEgress.fReplayCheck;
        DeviceSA_p->Params.IPsecEgress.fIgHdrInsert   =
            SA_p->Params.IPsecEgress.fIgHdrInsert;
        DeviceSA_p->Params.IPsecEgress.fUpdateUDP     =
            SA_p->Params.IPsecEgress.fUpdateUDP;
        DeviceSA_p->Params.IPsecEgress.fUpdateIP      =
            SA_p->Params.IPsecEgress.fUpdateIP;
        DeviceSA_p->Params.IPsecEgress.fNAT_UDP       =
            SA_p->Params.IPsecEgress.fNAT_UDP;
        DeviceSA_p->Params.IPsecEgress.fOuterIPHdr    =
            SA_p->Params.IPsecEgress.fOuterIPHdr;
        DeviceSA_p->Params.IPsecEgress.fConfProtect   =
            SA_p->Params.IPsecEgress.fConfProtect;
        DeviceSA_p->Params.IPsecEgress.fProtectFrames =
            SA_p->Params.IPsecEgress.fProtectFrames;
    }
    else if  (SA_p->ActionType == SECY_SA_ACTION_IPSEC_INGRESS)
    {
        DeviceSA_p->ActionType = EIP164_SECY_SA_ACTION_IPSEC_INGRESS;

        DeviceSA_p->Params.IPsecIngress.fRetainPad      =
            SA_p->Params.IPsecIngress.fRetainPad;
        DeviceSA_p->Params.IPsecIngress.fPadCheck       =
            SA_p->Params.IPsecIngress.fPadCheck;
        DeviceSA_p->Params.IPsecIngress.fReplayCheck    =
            SA_p->Params.IPsecIngress.fReplayCheck;
        DeviceSA_p->Params.IPsecIngress.fIgHdrInsert    =
            SA_p->Params.IPsecIngress.fIgHdrInsert;
        DeviceSA_p->Params.IPsecIngress.fReplayProtect  =
            SA_p->Params.IPsecIngress.fReplayProtect;
        DeviceSA_p->Params.IPsecIngress.fConfProtect    =
            SA_p->Params.IPsecIngress.fConfProtect;
        DeviceSA_p->Params.IPsecIngress.fPadNotValidDrop=
            SA_p->Params.IPsecIngress.fPadNotValidDrop;
        DeviceSA_p->Params.IPsecIngress.fPadLenFailDrop =
            SA_p->Params.IPsecIngress.fPadLenFailDrop;
        DeviceSA_p->Params.IPsecIngress.fUpdateIP       =
            SA_p->Params.IPsecIngress.fUpdateIP;
        DeviceSA_p->Params.IPsecIngress.fUpdateTTL      =
            SA_p->Params.IPsecIngress.fUpdateTTL;
    }
#endif
        ...
        unsigned int * const SCIndex_p)
{
    ...
    void *SCList = SecY_Device[DeviceId]->vPortDscr_p[vPort].SCList;
        ...
        const SABuilder_UpdCtrl_Params_t * const UpdateParams)
{
    ...
    if (Offset > 0)
    {
        ...
        Rc = EIP164_SecY_SA_Update_Control_Word_Update(&SecY_Device[DeviceId]->IOArea,
                                                       SAIndex,
                                                       Offset,
                                                       SAUpdateCtrlWord);
        ...
        const SecY_SA_t * const SA_p)
{
    ...
    SecY_vPort_Descriptor_t *vPortDsc_p = NULL;
#endif
...
#ifdef ADAPTER_EIP164_STRICT_ARGS
    if (vPort >= SecY_Device[DeviceId]->vPortCount)
    {
        SecYLib_Device_Unlock(DeviceId);
        return SECY_ERROR_BAD_PARAMETER;
    }
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
    vPortDsc_p = &SecY_Device[DeviceId]->vPortDscr_p[vPort];

    if (SA_p->ActionType == SECY_SA_ACTION_IPSEC_EGRESS ||
        SA_p->ActionType == SECY_SA_ACTION_IPSEC_INGRESS)
    {
        vPortDsc_p->ChannelMode = SECY_MODE_IPSEC;

#if defined(ADAPTER_EIP164_WARMBOOT_ENABLE)
        SecYLib_WarmBoot_ChMode_Set(DeviceId, vPort);
#endif
    }
    else
    {
        vPortDsc_p->ChannelMode = SECY_MODE_MACSEC;

#if defined(ADAPTER_EIP164_WARMBOOT_ENABLE)
        SecYLib_WarmBoot_ChMode_Clear(DeviceId, vPort);
#endif
    }
#endif
    ...
    fFirstSA = (SecY_Device[DeviceId]->vPortDscr_p[vPort].SACount == 0);

    if (fFirstSA)
    {
        LOG_INFO("\n\t EIP164_SecY_SAMFlow_Clear \n");

        Rc = EIP164_SecY_SAMFlow_Clear(&SecY_Device[DeviceId]->IOArea, vPort);
        if (Rc != EIP164_NO_ERROR)
        {
            SecYLib_Device_Unlock(DeviceId);
            return SECY_DLIB_ERROR(Rc);
    ...
    /* Obtain a free SA for this SecY device */
    {
        ...
        SAIndex = SADscr_p - SecY_Device[DeviceId]->SADscr_p;
        if (SAIndex >= SecY_Device[DeviceId]->SACount)
        {
        ...
        case SECY_SA_ACTION_CRYPT_AUTH:
            if (SecY_Device[DeviceId]->Role == SECY_ROLE_EGRESS)
        ...
        if (SA_p->SA_WordCount > 0)
        {
            ...
            Rc = EIP164_SecY_SA_Write(&SecY_Device[DeviceId]->IOArea,
                                      SAIndex,
...
#ifdef ADAPTER_EIP164_IPSEC_ENABLE
            if (SECY_SA_ACTION_IPSEC_INGRESS == SA_p->ActionType)
            {
                /* Write Replay Mask to device */
                Rc = EIP164_SecY_SA_ReplayMask_Write(
            ...
            (SADscr_p->u.InUse.MapType == SECY_SA_MAP_INGRESS_CRYPT_AUTH))
        {
            ...
            { /* Try to allocate new SC entry. */
...

                {
                    ...
                    for(;;)
                    {
                        ...
                        /* Add entry to RX CAM. */
                        Rc = EIP164_SecY_RxCAM_Add(&SecY_Device[DeviceId]->IOArea,
                                                   SCDscr_p->SCIndex,
                                                   (SCI[0] <<  0) |
                                                   (SCI[1] <<  8) |
                                                   (SCI[2] << 16) |
                ...
                Rc = EIP164_SecY_RxCAM_Stat_Clear(&SecY_Device[DeviceId]->IOArea,
                                                  SCDscr_p->SCIndex);
                if (Rc != EIP164_NO_ERROR)
                {
                    SecYLib_Device_Unlock(DeviceId);
            ...
            if (!SecY_SAHandle_IsSame(&SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].SAHandle[SADscr_p->u.InUse.AN],
                                      &SecY_SAHandle_NULL))
            {
                ...
                SADscr2_p = (SecY_SA_Descriptor_t *)SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].SAHandle[SADscr_p->u.InUse.AN].p;
                SADscr2_p->u.InUse.MapType = SECY_SA_MAP_DETACHED;
                SecY_Device[DeviceId]->vPortDscr_p[vPort].SACount--;
            }
            SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].SAHandle[SADscr_p->u.InUse.AN] = *SAHandle_p;
            ...
            if (SADscr_p->u.InUse.MapType == SECY_SA_MAP_INGRESS)
            {
                ...
                Rc = EIP164_SecY_SC_SA_Map_I_Update(&SecY_Device[DeviceId]->IOArea,
                                                    SADscr_p->u.InUse.SCIndex,
                                                    SADscr_p->u.InUse.AN,
                                                    SAIndex,
                                                    true,
        ...
        else
        {
            ...
            if ((SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].MapType !=
                 SECY_SA_MAP_DETACHED) &&
                !SecY_SAHandle_IsSame(&SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].SAHandle[0],
                                      &SecY_SAHandle_NULL))
            {
                ...
                SADscr2_p = (SecY_SA_Descriptor_t*)SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].SAHandle[0].p;
                SADscr2_p->u.InUse.MapType = SECY_SA_MAP_DETACHED;
                SecY_Device[DeviceId]->vPortDscr_p[vPort].SACount--;
            }
            SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].SAHandle[0] = *SAHandle_p;
            SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].MapType = SADscr_p->u.InUse.MapType;
            ...
            if (SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].MapType ==
                SECY_SA_MAP_EGRESS)
            {
                if (SecY_Device[DeviceId]->vPortDscr_p[vPort].MTU != 0)
                {
            ...
            if (SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].MapType !=
                SECY_SA_MAP_DETACHED)
            {
                Rc = EIP164_SecY_SC_SA_Map_E_Update(&SecY_Device[DeviceId]->IOArea,
                                                    SADscr_p->u.InUse.SCIndex,
                                                    SAIndex,
                                                    true,
                                                    false);
    ...
    if (fFirstSA)
    {
        ...
        Rc = EIP164_SecY_SAMFlow_Write(&SecY_Device[DeviceId]->IOArea,
                                       vPort,
                                       &DeviceSA
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
                                       , vPortDsc_p->ChannelMode
#endif
                                       );
    ...
    if (SA_p->ActionType == SECY_SA_ACTION_EGRESS
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
        || SA_p->ActionType == SECY_SA_ACTION_IPSEC_EGRESS
#endif
       )
    {
        ...
        Rc = EIP164_SecY_SA_Stat_E_Clear(&SecY_Device[DeviceId]->IOArea,
                                         SAIndex);
        if (Rc != EIP164_NO_ERROR)
        {
       ...
       )
    {
        ...
        Rc = EIP164_SecY_SA_Stat_I_Clear(&SecY_Device[DeviceId]->IOArea,
                                         SAIndex);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        const SecY_SA_t * const SA_p)
{
    ...
    Rc = EIP164_SecY_SAMFlow_Write(&SecY_Device[DeviceId]->IOArea,
                                   ...
                                   &DeviceSA
#if defined(ADAPTER_EIP164_IPSEC_ENABLE)
                                   , SecY_Device[DeviceId]->vPortDscr_p->ChannelMode
#endif
                                   );
        ...
        bool * const fNextPNWritten_p)
{
    ...
    Rc = EIP164_SecY_SA_NextPN_Update(&SecY_Device[DeviceId]->IOArea,
                                      SAIndex,
                                      NextPN_Lo,
                                      NextPN_Hi,
                                      fExtPN);
    ...
    do
    {
        ...
        Rc = EIP164_SecY_SA_NextPN_Status_Get(&SecY_Device[DeviceId]->IOArea,
                                              &fNextPNWritten,
                                              &fBusy);
        if (Rc != EIP164_NO_ERROR)
        {
        ...
        const SecY_SAHandle_t SAHandle)
{
    ...
    if (SAIndex >= SecY_Device[DeviceId]->SACount ||
        vPort >= SecY_Device[DeviceId]->vPortCount)
    {
        SecYLib_Device_Unlock(DeviceId);
        return SECY_ERROR_INTERNAL;
    }
    ...
    if (SADscr_p->u.InUse.MapType != SECY_SA_MAP_DETACHED)
    {
        ...
        if (SADscr_p->u.InUse.SCIndex >= SecY_Device[DeviceId]->SCCount)
            goto sa_remove_error;

        SCDscr_p = &SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex];
            ...
            (SADscr_p->u.InUse.MapType == SECY_SA_MAP_INGRESS_CRYPT_AUTH))
        {
...
#if defined(ADAPTER_EIP164_IPSEC_ENABLE) && defined(ADAPTER_EIP164_WARMBOOT_ENABLE)
            if (SECY_MODE_IPSEC ==
                SecY_Device[DeviceId]->vPortDscr_p[vPort].ChannelMode)
            {
                SecYLib_WarmBoot_ChMode_Clear(DeviceId, vPort);
            }
#endif
                ...
                SecYLib_SAHandle_IsSame(&SCDscr_p->SAHandle[3], &SecY_SAHandle_NULL))
            {
                ...
                List_Rc = List_RemoveAnywhere(0,
                                              SecY_Device[DeviceId]->vPortDscr_p[vPort].SCList,
                                              &SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex].OnList);
                if (List_Rc != LIST_STATUS_OK)
                {
...

                {
                    ...
                    for(;;)
                    {
                        ...
                        Rc = EIP164_SecY_RxCAM_Remove(&SecY_Device[DeviceId]->IOArea,
                                                      SADscr_p->u.InUse.SCIndex);
                        if (Rc == EIP164_NO_ERROR)
                        {
                            break;
        ...
        SecY_Device[DeviceId]->vPortDscr_p[vPort].SACount--;
        if (SecY_Device[DeviceId]->vPortDscr_p[vPort].SACount==0)
        {
            LOG_INFO("\n\t EIP164_SecY_SAMFlow_Clear \n");

            Rc = EIP164_SecY_SAMFlow_Clear(&SecY_Device[DeviceId]->IOArea,
                                           vPort);
            if (Rc != EIP164_NO_ERROR)
            {
...
sa_remove_error:
    /* Add SA to be removed to the SA free list */
    {
        List_Status_t List_Rc;
        /* Invalidate record. */
        SecY_Device[DeviceId]->SADscr_p[SAIndex].Magic = 0;

        SecY_Device[DeviceId]->SADscr_p[SAIndex].u.free.DataObject_p =
            &SecY_Device[DeviceId]->SADscr_p[SAIndex];

        List_Rc = List_AddToHead(0,
                                 SecY_Device[DeviceId]->SAFL_p,
                                 &SecY_Device[DeviceId]->SADscr_p[SAIndex].u.free);
        if (List_Rc != LIST_STATUS_OK)
        {
        ...
        const SecY_SA_t * const NewSA_p)
{
    ...
    /* Obtain a free SA for this SecY device */
    {
        ...
        SecY_SC_Descriptor_t *SCDscr_p = &SecY_Device[DeviceId]->SCDscr_p[SADscr_p->u.InUse.SCIndex];
        List_Status_t List_Rc = List_RemoveFromTail(0,
                                                    SecY_Device[DeviceId]->SAFL_p,
                                                    &Element_p);
        if (List_Rc != LIST_STATUS_OK || Element_p->DataObject_p == NULL)
        ...
        NewSAIndex = NewSADscr_p - SecY_Device[DeviceId]->SADscr_p;
    ...
    if (SecY_Device[DeviceId]->vPortDscr_p[ActiveSCIndex].MTU != 0)
    {
        /* Set MTU value in new SA. */
        SecY_Rc = SecYLib_SA_MTU_Update(DeviceId,
                                        NewSAIndex,
                                        SecY_Device[DeviceId]->vPortDscr_p[ActiveSCIndex].MTU);
        if (SecY_Rc != SECY_STATUS_OK)
        {
            LOG_CRIT("%s: Failed to update MTU word for device %d "
                     "(%s), error %d\n",
    ...
    Rc = EIP164_SecY_SA_Installation_Verify(&SecY_Device[DeviceId]->IOArea,
                                       SAMFlowCtrlIndex,
                                       &SAIndex,
                                       &fSAInUse);
        ...
        SecY_SAHandle_t * const ActiveSAHandle_p)
{
    ...
    Rc = EIP164_SecY_SA_Active_I_Get(&SecY_Device[DeviceId]->IOArea,
                                SCIndex,
                                SAIndex,
                                fSAInUse);
    if (Rc != EIP164_NO_ERROR)
        ...
        const SecY_SC_Rule_MTUCheck_t * const MTUCheck_Rule_p)
{
    ...
    if (SCIndex >= SecY_Device[DeviceId]->SCCount)
        return SECY_ERROR_BAD_PARAMETER;
    ...
    SAB_Rc = SABuilder_MTU_Update(&MTUParams, &SecY_Device[DeviceId]->vPortDscr_p[SCIndex].MTU);
    if (SAB_Rc != SAB_STATUS_OK)
    {
        SecYLib_Device_Unlock(DeviceId);
        return SECY_ERROR_INTERNAL;
    ...
    while (fSAInUse)
    {
        ...
        SecY_Rc = SecYLib_SA_MTU_Update(DeviceId,
                                        SAIndex,
                                        SecY_Device[DeviceId]->vPortDscr_p[SCIndex].MTU);
        if (SecY_Rc != SECY_STATUS_OK)
        {
            SecYLib_Device_Unlock(DeviceId);
            return SECY_DLIB_ERROR(Rc);
        ...
        SecY_SC_Rule_MTUCheck_t * const MTUCheck_Rule_p)
{
    ...
    MTU = SecY_Device[DeviceId]->vPortDscr_p[SCIndex].MTU;
