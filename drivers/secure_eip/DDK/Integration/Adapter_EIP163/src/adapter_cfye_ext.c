        ...
        CfyE_Role_t * const Role_p)
{
    ...
    *Role_p = CfyE_Devices[DeviceId]->Role;
        ...
        CfyE_vPort_t * const vPort_p)
{
    ...
    EIP163_Rc = EIP163_vPortPolicy_Read(&CfyE_Devices[DeviceId]->IOArea,
        ...
        CfyE_vPortHandle_t * const NextvPortHandle_p)
{
    ...
    for ( ; vPortId < CfyE_Devices[DeviceId]->vPortCount; vPortId++)
    {
        if (CfyE_Devices[DeviceId]->vPortDscr_p[vPortId].Magic ==
        ...
        bool * const fEnabled_p)
{
    ...
    CfyE_vPort_Descriptor_Internal_t *vPortDscr = NULL;
    ...
    vPortDscr = CfyE_Devices[DeviceId]->RuleDscr_p[RuleId].u.InUse.vPortDscr_p;
...

    {
...

        {
            ...
            for(;;)
            {
                ...
                EIP163_Rc = EIP163_Rule_Read(&CfyE_Devices[DeviceId]->IOArea,
                                             RuleId,
                                             &DeviceRule,
                                             fEnabled_p,
                                             ChannelMode);
        ...
        CfyE_RuleHandle_t * const NextRuleHandle_p)
{
    ...
    for ( ; RuleId < CfyE_Devices[DeviceId]->RulesCount; RuleId++)
    {
        if (CfyE_Devices[DeviceId]->RuleDscr_p[RuleId].Magic ==
        ...
        CfyE_RuleHandle_t * const NextRuleHandle_p)
{
    ...
    for ( ; RuleId < CfyE_Devices[DeviceId]->RulesCount; RuleId++)
    {
        if (CfyE_Devices[DeviceId]->RuleDscr_p[RuleId].Magic ==
            ADAPTER_EIP163_RULE_DSCR_MAGIC &&
            CfyE_Devices[DeviceId]->RuleDscr_p[RuleId].u.InUse.vPortDscr_p ==
            vPortDscr_p)
        {
        ...
        bool * const fEnabled_p)
{
    ...
    EIP163_Rc = EIP163_MTT_Read(&CfyE_Devices[DeviceId]->IOArea,
                                MTTIndex,
                                &DeviceMTT,
                                fEnabled_p);
    if (EIP163_Rc != EIP163_NO_ERROR)
    {
        LOG_CRIT("%s: Failed to read MTT rule %d "
                 "for EIP-163 device %d (%s), error %d\n",
                 __func__,
                 MTTIndex,
                 DeviceId,
    ...
    memcpy(&MTT_p->Key.ChannelMask, &DeviceMTT.Key.ChannelMask, sizeof(EIP163_Ch_Mask_t));

    MTT_p->Key.fIPv6       = DeviceMTT.Key.fIPv6;
    MTT_p->Key.TagLabel1   = DeviceMTT.Key.TagLabel1;
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
    MTT_p->Key.fIPHdrValid = DeviceMTT.Key.fIPHdrValid;
    MTT_p->Key.TagLabel2   = DeviceMTT.Key.TagLabel2;
    MTT_p->Key.fPacketType = DeviceMTT.Key.fPacketType;
#endif
    memcpy(MTT_p->IPAddr, DeviceMTT.IPAddr, 16);

    memcpy(&MTT_p->Mask.ChannelMask, &DeviceMTT.Mask.ChannelMask, sizeof(EIP163_Ch_Mask_t));

    MTT_p->Mask.fIPv6       = DeviceMTT.Mask.fIPv6;
    MTT_p->Mask.TagLabel1   = DeviceMTT.Mask.TagLabel1;
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
    MTT_p->Mask.fIPHdrValid = DeviceMTT.Mask.fIPHdrValid;
    MTT_p->Mask.TagLabel2   = DeviceMTT.Mask.TagLabel2;
    MTT_p->Mask.fPacketType = DeviceMTT.Mask.fPacketType;
#endif
    memcpy(MTT_p->IPAddrMask, DeviceMTT.IPAddrMask, 16);
        ...
        const unsigned int DeviceId)
{
    ...
    /* CfyE_Device_Limits */
    {
       ...
       Log_FormattedMessage("IPsec support: %d\n",Device_Limits.fIPsec);
#endif
#ifdef ADAPTER_EIP163_VXLAN_ENABLE
       Log_FormattedMessage("VxLAN support: %d\n",Device_Limits.fVxLAN);
#endif

#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
       Log_FormattedMessage("MTT entries=%d\n",Device_Limits.mtt_count);
#endif
    }
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        CfyE_Rc = CfyE_Statistics_Summary_MTT_Read(DeviceId,
                                                   &MTT_ChannelSummary);
        if (CfyE_Rc != CFYE_STATUS_OK)
        {
            LOG_CRIT("CfyE_Statistics_Summary_MTT_Read returned error %d\n",
                     CfyE_Rc);
            return CfyE_Rc;
        }
        Log_FormattedMessage("MTT Summary 0x%08x\n",
                             MTT_ChannelSummary);
#endif
    }
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        CfyE_IPSEC_Parser_t IPsec_Parser;
#endif
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        HeaderParser.IPsec_Parser_p = &IPsec_Parser;
#endif
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
...
#if defined(ADAPTER_EIP163_VXLAN_ENABLE)
       DevCtrl.L3L4_p = &L3L4;
       HeaderParser.L4_Ports_p = &L4_Ports;
#endif
        ...
        Log_FormattedMessage(
            "DevCtrl fLowLatencyBypass=%d fDef a u l tVPortValid=%d"
            " Def a u l tVPort=%u fShouldSecure=%d\n"
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        Log_FormattedMessage(
            "EgressHeader EgressHeaderEtype=0x%04x fEnable=%d\n",
            EgressHeader.EgressHeaderEtype,
            EgressHeader.fEnable);
#endif
        Log_FormattedMessage(
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        Log_FormattedMessage(
            "MPLS Parser MPLS3_Select1 = %d MPLS3_Select2 = %d\n"
            "            MPLS4_Select1 = %d MPLS4_Select2 = %d\n"
            "            MPLS5_Select1 = %d MPLS5_Select2 = %d\n",
            MPLS_Parser.MPLS3_Select1,
            MPLS_Parser.MPLS3_Select2,
            MPLS_Parser.MPLS4_Select1,
            MPLS_Parser.MPLS4_Select2,
            MPLS_Parser.MPLS5_Select1,
            MPLS_Parser.MPLS5_Select2);
#endif

#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        for (i = 0; i < CFYE_MPLS_ETYPE_MAX_COUNT; i++)
        {
            Log_FormattedMessage(" MPLS %d Etype=0x%04x fCompare=%d\n",
                                 i,
        ...
        Log_FormattedMessage(
            "VLAN Parser Def a u l tUp=%d QTag=0x%04x"
...
#if defined(ADAPTER_EIP163_VXLAN_ENABLE)
        Log_FormattedMessage("L3L4Flags 0x%08x\n",L3L4.flags);
#endif
#ifdef ADAPTER_EIP163_VXLAN_ENABLE
       Log_FormattedMessage(
           "VxLAN Ports=%d,%d\n",
           L4_Ports.VxLAN_Port1,
           L4_Ports.VxLAN_Port2);
#endif

#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        Log_FormattedMessage(
            "           MTTMiss                 lo=%10u hi=%10u\n",
            Chan_Stat.MTTMiss.Lo,
            Chan_Stat.MTTMiss.Hi);
#endif
    }
        ...
        const bool fIncludeRule)
{
    ...
    else
    {
        ...
        Log_FormattedMessage(" PltExtension=%d SecTagOffset=%d\n",
        ...
        CfyE_Rule_PacketType_t t)
{
    ...
    case CFYE_RULE_PKT_TYPE_OTHER:
        return "OTHER";
    case CFYE_RULE_PKT_TYPE_MPLS:
        return "MPLS";
    case CFYE_RULE_PKT_TYPE_IPSEC:
        return "IPSEC";
    case CFYE_RULE_PKT_TYPE_IPSEC_MPLS:
        return "IPSEC_MPLS";
    case CFYE_RULE_PKT_TYPE_IPSEC_UDP:
        return "IPSEC_UDP";
    default:
        return "Unknown";
    }
        ...
        const bool fAllMTT)
{
    ...
    if (fAllMTT)
    {
        ...
        for (i=0; i<Device_Limits.mtt_count; i++)
        {
            CfyE_Diag_MTT_Dump(DeviceId, i, false);
        }
    }
    else
    {
        CfyE_MTT_t MTT;
        CfyE_Statistics_MTT_t MTT_Stat;
        bool fEnabled;
        unsigned int MaxCfyEChannels = 0;
        uint16_t i = 0;
        ...
        Log_FormattedMessage("CfyE_Diag_MTT #%d\n",MTTIndex);
        CfyE_Rc = CfyE_MTT_Read(DeviceId, MTTIndex, &MTT, &fEnabled);

        if (CfyE_Rc != CFYE_STATUS_OK)
        {
            LOG_CRIT("CfyE_MTT_Read returned error %d\n",CfyE_Rc);
            return CfyE_Rc;
        }

        Log_FormattedMessage("Enabled=%d IPv6=%d (mask=%d) TagLabel=0x%x Mask=0x%x\n",
                             fEnabled,
                             MTT.Key.fIPv6,
                             MTT.Mask.fIPv6,
                             MTT.Key.TagLabel1,
        ...
        for (i = 0; i < ((MaxCfyEChannels + 31) / 32); i++)
            Log_FormattedMessage("ChannelMask = 0x%08x, (mask = 0x%08x)\n",
                                 MTT.Key.ChannelMask.ch_bitmask[i],
                                 MTT.Mask.ChannelMask.ch_bitmask[i]);

        Log_FormattedMessage("\n");

        Log_HexDump32("IPAddr     ",
                      0,
                      MTT.IPAddr,
                      4);
        Log_HexDump32("IPAddrMask ",
                      0,
                      MTT.IPAddrMask,
                      4);
        CfyE_Rc = CfyE_Statistics_MTT_Get(DeviceId,
                                           MTTIndex,
                                           &MTT_Stat,
                                           false);
        if (CfyE_Rc != CFYE_STATUS_OK)
        {
            LOG_CRIT("CfyE_Statistics_MTT_Get returned error %d\n",CfyE_Rc);
            return CfyE_Rc;
        }
        Log_FormattedMessage("MTT Stats Counter lo=%10u hi=%10u\n",
                             MTT_Stat.Counter.Lo,
                             MTT_Stat.Counter.Hi);
    }
    return CFYE_STATUS_OK;
