...
#include "basic_defs.h"             /* uint8_t, MASK_n_BITS */
...
/* Number of bytes for the WarmBoot ChMode bitmap */
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
#define CFYE_WARMBOOT_CH_MODE_BYTE_COUNT(n) (((n)+7)/8)
#endif
        ...
        unsigned int vPortIndex)
{
    ...
    CfyE_Devices[DeviceId]->WarmBoot_ChMode_Bitmap[ByteIndex] |= BitMask;

    WarmBootLib_Write(CfyE_Devices[DeviceId]->WarmBoot_AreaId,
                      CfyE_Devices[DeviceId]->WarmBoot_ChMode_Bitmap + ByteIndex,
                      CFYE_WARMBOOT_BASE_BYTE_COUNT +
                      CFYE_WARMBOOT_VPORT_BYTE_COUNT(CfyE_Devices[DeviceId]->vPortCount) +
        ...
        unsigned int vPortIndex)
{
    ...
    return (CfyE_Devices[DeviceId]->WarmBoot_ChMode_Bitmap[ByteIndex] & BitMask) != 0;
}
#endif
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        + CFYE_WARMBOOT_CH_MODE_BYTE_COUNT(Cp.EIP163_Options.vPort_Count)
#endif
        ;
        ...
        unsigned int DeviceId)
{
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
    LOG_CRIT("%s: Allocating the bitmap for WarmBoot Channel Mode bitmap. "
                 "for device %d\n",
                 __func__, DeviceId);

    CfyE_Devices[DeviceId]->WarmBoot_ChMode_Bitmap =
        Adapter_Alloc(CFYE_WARMBOOT_CH_MODE_BYTE_COUNT(CfyE_Devices[DeviceId]->vPortCount));

    if (CfyE_Devices[DeviceId]->WarmBoot_ChMode_Bitmap == NULL)
    {
    ...
    if ( !WarmBootLib_Alloc(CFYE_WARMBOOT_BASE_BYTE_COUNT +
                            ...
                            CFYE_WARMBOOT_RULE_BYTE_COUNT(CfyE_Devices[DeviceId]->RulesCount)
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
                            + CFYE_WARMBOOT_CH_MODE_BYTE_COUNT(CfyE_Devices[DeviceId]->vPortCount)
#endif
                            , &CfyE_Devices[DeviceId]->WarmBoot_AreaId))
    {
        ...
        Adapter_Free(CfyE_Devices[DeviceId]->WarmBoot_Rule_Bitmap);
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        Adapter_Free(CfyE_Devices[DeviceId]->WarmBoot_ChMode_Bitmap);
#endif
        return false;
    }
    ...
    memset(CfyE_Devices[DeviceId]->WarmBoot_vPort_Bitmap, 0,
           CFYE_WARMBOOT_VPORT_BYTE_COUNT(CfyE_Devices[DeviceId]->vPortCount));
    memset(CfyE_Devices[DeviceId]->WarmBoot_Rule_Bitmap, 0,
           CFYE_WARMBOOT_RULE_BYTE_COUNT(CfyE_Devices[DeviceId]->RulesCount));
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
    memset(CfyE_Devices[DeviceId]->WarmBoot_ChMode_Bitmap, 0,
           CFYE_WARMBOOT_CH_MODE_BYTE_COUNT(CfyE_Devices[DeviceId]->vPortCount));
#endif
        ...
        unsigned int AreaId)
{
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
    CfyE_Devices[DeviceId]->WarmBoot_ChMode_Bitmap =
        Adapter_Alloc(CFYE_WARMBOOT_CH_MODE_BYTE_COUNT(vPortCount));
    ...
    WarmBootLib_Read(AreaId,
                     ...
                     CFYE_WARMBOOT_VPORT_BYTE_COUNT(vPortCount) +
                     CFYE_WARMBOOT_RULE_BYTE_COUNT(RulesCount),
                     CFYE_WARMBOOT_CH_MODE_BYTE_COUNT(vPortCount));
#endif
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
                if (CfyELib_WarmBoot_ChMode_Get(DeviceId, i) == 0)
                {
                    CfyE_Devices[DeviceId]->vPortDscr_p[i].u.InUse.ChannelMode = CYFE_MODE_MACSEC;
                }
                else
                {
                    CfyE_Devices[DeviceId]->vPortDscr_p[i].u.InUse.ChannelMode = CYFE_MODE_IPSEC;
                }
#endif
