...
#include "adapter_cfye_support.h"   /* Typedefs */
        ...
        const CfyE_Init_t * const Init_p)
{
    ...
    Settings.MTTCountFrameThrLo          = Init_p->MTTCountFrameThrLo;
    Settings.MTTCountFrameThrHi          = Init_p->MTTCountFrameThrHi;
#endif
#ifdef ADAPTER_EIP163_IPSEC_ENABLE
    Settings.fParsePktIPsec              = Init_p->fParsePktIPsec;
#endif
    Settings.CountIncDisCtrl             = Init_p->CountIncDisCtrl;
    Settings.EOPTimeoutVal               = Init_p->EOPTimeoutVal;
    memcpy(&Settings.EOPTimeoutCtrl, &Init_p->EOPTimeoutCtrl, sizeof(CfyE_Ch_Mask_t));
    ...
    Rc = EIP163_Device_Init(&CfyE_Devices[DeviceId]->IOArea,
                            Device,
                            &Settings);
    ...
    if (Rc == EIP163_BUSY_RETRY_LATER)
    {
        ...
        do
        {
            ...
            Rc = EIP163_Device_Is_Done(&CfyE_Devices[DeviceId]->IOArea);
            if (Rc != EIP163_BUSY_RETRY_LATER)
                break;
    ...
    Rc = EIP163_Device_Clocks_Off(&CfyE_Devices[DeviceId]->IOArea,
                             EIP163_ALL_CLOCKS);
    if (Rc != EIP163_NO_ERROR)
    {
        Adapter_Free(CfyE_Devices[DeviceId]);
    ...
    Rc = EIP163_Device_Clocks_Off(&CfyE_Devices[DeviceId]->IOArea, 0);
    if (Rc != EIP163_NO_ERROR)
    {
        Adapter_Free(CfyE_Devices[DeviceId]);
        CfyE_Devices[DeviceId] = NULL;
    ...
    Rc = EIP163_Device_Clocks_On(&CfyE_Devices[DeviceId]->IOArea,
                            EIP163_ALL_CLOCKS,
                            1);
    if (Rc != EIP163_NO_ERROR)
    {
    ...
    CfyE_Devices[DeviceId]->RulesCount    = Cp.EIP163_Options.Rules_Count;
    CfyE_Devices[DeviceId]->vPortCount    = Cp.EIP163_Options.vPort_Count;
    CfyE_Devices[DeviceId]->ChannelsCount = Cp.EIP163_Options.Channel_Count;
    ...
    CfyE_Devices[DeviceId]->fExternalTCAM = Cp.EIP163_Options.fExternalTCAM;

    /* Restrict any counts to the values specified by Init_p */
    if (Init_p->MaxChannelCount > 0 &&
        Init_p->MaxChannelCount < CfyE_Devices[DeviceId]->ChannelsCount)
    {
        LOG_CRIT("%s: restricting ChannelsCount to %u\n",
                 __func__,Init_p->MaxChannelCount);
        CfyE_Devices[DeviceId]->ChannelsCount = Init_p->MaxChannelCount;
    }
    if (Init_p->MaxvPortCount > 0 &&
        Init_p->MaxvPortCount < CfyE_Devices[DeviceId]->vPortCount)
    {
        LOG_CRIT("%s: restricting vPortCount to %u\n",
                 __func__,Init_p->MaxvPortCount);
        CfyE_Devices[DeviceId]->vPortCount = Init_p->MaxvPortCount;
    }
    if (Init_p->MaxRuleCount > 0 &&
        Init_p->MaxRuleCount < CfyE_Devices[DeviceId]->RulesCount)
    {
        LOG_CRIT("%s: restricting RulesCount to %u\n",
                 __func__,Init_p->MaxRuleCount);
        CfyE_Devices[DeviceId]->RulesCount = Init_p->MaxRuleCount;
    }
    ...
    if (!CfyELib_DataObjectList_Init(
                                ...
                                false,
                                (void**)&CfyE_Devices[DeviceId]->vPortDscr_p,
                                CfyE_Devices[DeviceId]->vPortCount))
    {
    ...
    if (!CfyELib_DataObjectList_Init(
                                ...
                                true,
                                (void**)&CfyE_Devices[DeviceId]->RuleDscr_p,
                                CfyE_Devices[DeviceId]->RulesCount))
    {
        ...
        const unsigned int DeviceId)
{
    ...
    List_GetListElementCount(0,
                             CfyE_Devices[DeviceId]->vPortFreeList_p,
                             &Count);
    if (Count != CfyE_Devices[DeviceId]->vPortCount)
    {
        LOG_CRIT("%s: EIP-163 device %d (%s) has %d added vPorts\n",
                 __func__,
                 DeviceId,
                 ADAPTER_EIP163_DEVICE_NAME(DeviceId),
                 CfyE_Devices[DeviceId]->vPortCount - Count);
        CfyELib_Device_Unlock(DeviceId);
        return CFYE_ERROR_INTERNAL;
    }
    ...
    Rc = EIP163_Device_Uninit(&CfyE_Devices[DeviceId]->IOArea);
    if (Rc != EIP163_NO_ERROR && Rc != EIP163_BUSY_RETRY_LATER)
    {
        ...
        CfyE_Device_Limits_t* const device_limits_p)
{
    ...
    device_limits_p->channel_count = CfyE_Devices[DeviceId]->ChannelsCount;
    device_limits_p->vport_count = CfyE_Devices[DeviceId]->vPortCount;
    device_limits_p->rule_count = CfyE_Devices[DeviceId]->RulesCount;
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
    device_limits_p->mtt_count = cp.EIP163_Options.MTT_Count;
#endif
#ifdef ADAPTER_EIP163_VXLAN_ENABLE
    device_limits_p->fVxLAN = cp.EIP163_Options.fVxLAN;
#endif
#ifdef ADAPTER_EIP163_IPSEC_ENABLE
    device_limits_p->fIPsec = cp.EIP163_Options.fIPSecSupport;
#endif
    LOG_INFO("\n %s done\n", __func__);
    CfyELib_Device_Unlock(DeviceId);
        ...
        unsigned int * const MaxRuleCount_p)
{
    ...
    if (MaxChannelCount_p != NULL)
        *MaxChannelCount_p = CfyE_Devices[DeviceId]->ChannelsCount;

    if (MaxvPortCount_p != NULL)
        *MaxvPortCount_p = CfyE_Devices[DeviceId]->vPortCount;

    if (MaxRuleCount_p != NULL)
        *MaxRuleCount_p = CfyE_Devices[DeviceId]->RulesCount;
        ...
        const CfyE_Device_t * const Device_p)
{
    ...
    if (Device_p->Control_p)
    {
...
#if defined(ADAPTER_EIP163_VXLAN_ENABLE)
        EIP163_Channel_L3L4_t L3L4;
#endif
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        DeviceControl.fIPSec = DevCtrl_p->fIPsec;
#endif
        ...
        if (DevCtrl_p->Exceptions_p)
        {
            ...
            DeviceEx.ECCDropAction      = DevCtrl_p->Exceptions_p->ECCDropAction;
       ...
       if (DevCtrl_p->L3L4_p)
       {
           L3L4.flags = DevCtrl_p->L3L4_p->flags;
           DeviceControl.L3L4_p = &L3L4;
       }
#endif
        ...
        EIP163_Rc = EIP163_Device_Update(&CfyE_Devices[DeviceId]->IOArea,
                                         ChannelId,
                                         &DeviceControl);
    ...
    if (Device_p->CP_p)
    {
        ...
        EIP163_Rc = EIP163_ControlPacket_Update(&CfyE_Devices[DeviceId]->IOArea,
                                                ChannelId,
                                                &DeviceCP);
        if (EIP163_Rc != EIP163_NO_ERROR)
        {
    ...
    {
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        EIP163_EgressHeader_t   EgressHeader;
#endif
        EIP163_SecTAG_Parser_t  SecTag;
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        EIP163_IPsec_Parser_t  IPsec_Parser;
#endif
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        EIP163_MPLS_Parser_t    MPLS_Parser;
#endif
        EIP163_VLAN_Parser_t    VLAN_Parser;
#if defined(ADAPTER_EIP163_VXLAN_ENABLE)
        EIP163_L4_Ports_t       L4_Ports;
#endif
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        /* Egress header parser control settings */
        if (HdrPrs_p->EgressHeader_p)
        {
            CfyE_EgressHeader_t * EHdr_p = HdrPrs_p->EgressHeader_p;

            ZEROINIT(EgressHeader);

            EgressHeader.EgressHeaderEtype = EHdr_p->EgressHeaderEtype;
            EgressHeader.fEnable           = EHdr_p->fEnable;

            HdrParserControl.EgressHeader_p = &EgressHeader;
        }
#endif
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        /* MPLS parser control settings */
        if (HdrPrs_p->MPLS_Parser_p)
        {
            unsigned int i;
            CfyE_MPLS_Parser_t * MPLS_p = HdrPrs_p->MPLS_Parser_p;

            ZEROINIT(MPLS_Parser);

            MPLS_Parser.MPLS3_Select1 = MPLS_p->MPLS3_Select1;
            MPLS_Parser.MPLS3_Select2 = MPLS_p->MPLS3_Select2;
            MPLS_Parser.MPLS4_Select1 = MPLS_p->MPLS4_Select1;
            MPLS_Parser.MPLS4_Select2 = MPLS_p->MPLS4_Select2;
            MPLS_Parser.MPLS5_Select1 = MPLS_p->MPLS5_Select1;
            MPLS_Parser.MPLS5_Select2 = MPLS_p->MPLS5_Select2;
#endif
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
            for (i = 0; i < CFYE_MPLS_ETYPE_MAX_COUNT; i++)
            {
                MPLS_Parser.MPLS_Etype[i].fCompare =
                                        MPLS_p->MPLS_Etype[i].fCompare;
                MPLS_Parser.MPLS_Etype[i].MPLS_Etype =
                                        MPLS_p->MPLS_Etype[i].MPLS_Etype;
            }

            HdrParserControl.MPLS_Parser_p = &MPLS_Parser;
        }
#endif


#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        /* IPSec parser control settings */
        if (HdrPrs_p->IPsec_Parser_p)
        {
            CfyE_IPSEC_Parser_t * IPSecHdr_p = HdrPrs_p->IPsec_Parser_p;

            ZEROINIT(IPsec_Parser);

            IPsec_Parser.fAllowFragments = IPSecHdr_p->fAllowFragments;
            IPsec_Parser.fIgnoreIPv4ChkSum = IPSecHdr_p->fIgnoreIPv4ChkSum;
            IPsec_Parser.fSubParseIKE = IPSecHdr_p->fSubParseIKE;
            IPsec_Parser.fSubParseNATIKE = IPSecHdr_p->fSubParseNATIKE;
            IPsec_Parser.fSubParseNATKeepAlive = IPSecHdr_p->fSubParseNATKeepAlive;

            IPsec_Parser.fMACDACheck = IPSecHdr_p->fMACDACheck;
            IPsec_Parser.fParseESP = IPSecHdr_p->fParseESP;
            IPsec_Parser.fParseIKE = IPSecHdr_p->fParseIKE;
            IPsec_Parser.fParseIP = IPSecHdr_p->fParseIP;
            IPsec_Parser.fParseNAT = IPSecHdr_p->fParseNAT;
            IPsec_Parser.fParseNATIKE = IPSecHdr_p->fParseNATIKE;
            IPsec_Parser.fParseNATKeepAlive = IPSecHdr_p->fParseNATKeepAlive;
            IPsec_Parser.fParseUDP = IPSecHdr_p->fParseUDP;
            IPsec_Parser.fVerifyUDPChkSum = IPSecHdr_p->fVerifyUDPChkSum;

            IPsec_Parser.IKE_Port = IPSecHdr_p->IKE_Port;
            IPsec_Parser.NAT_Port = IPSecHdr_p->NAT_Port;

            memcpy(IPsec_Parser.MAC_DA, IPSecHdr_p->MAC_DA, 6);

            HdrParserControl.IPsec_Parser_p = &IPsec_Parser;
        }
#endif
...
#if defined(ADAPTER_EIP163_VXLAN_ENABLE)
        if (HdrPrs_p->L4_Ports_p)
        {
#ifdef ADAPTER_EIP163_VXLAN_ENABLE
            L4_Ports.VxLAN_Port1 = HdrPrs_p->L4_Ports_p->VxLAN_Port1;
            L4_Ports.VxLAN_Port2 = HdrPrs_p->L4_Ports_p->VxLAN_Port2;
#endif
            HdrParserControl.L4_Ports_p = &L4_Ports;
        }
        ...
        EIP163_Rc = EIP163_HeaderParser_Update(&CfyE_Devices[DeviceId]->IOArea,
                                               ChannelId,
                                               &HdrParserControl);
        if (EIP163_Rc != EIP163_NO_ERROR)
        {
    ...
    if (Device_p->StatControl_p)
    {
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        StatControl.MTTCountFrameThr.low  = StatCtrl_p->MTTCountFrameThr.Lo;
        StatControl.MTTCountFrameThr.hi   = StatCtrl_p->MTTCountFrameThr.Hi;
#endif
        ...
        EIP163_Rc = EIP163_StatisticsControl_Update(&CfyE_Devices[DeviceId]->IOArea,
                                                    &StatControl);
        ...
        if (EIP163_Rc == EIP163_BUSY_RETRY_LATER)
        {
            ...
            do
            {
                ...
                EIP163_Rc = EIP163_Device_Is_Done(&CfyE_Devices[DeviceId]->IOArea);
                if (EIP163_Rc != EIP163_BUSY_RETRY_LATER)
                {
                    break;
                }
    ...
    if (Device_p->ECCConf_p)
    {
        ...
        EIP163_Rc = EIP163_Device_ECC_Configure(&CfyE_Devices[DeviceId]->IOArea,
        ...
        CfyE_Device_t * const Device_p)
{
    ...
    if (DevCtrl_p)
    {
        ...
        Rc = EIP163_Device_Control_Read(&CfyE_Devices[DeviceId]->IOArea,
                                         ChannelId,
                                         &DeviceControl);
        if (Rc != EIP163_NO_ERROR)
        {
...
#if defined(ADAPTER_EIP163_IPSEC_ENABLE)
        DevCtrl_p->fIPsec = DeviceControl.fIPSec;
#else
        DevCtrl_p->fIPsec = false;
#endif
        ...
        if (DevCtrl_p->Exceptions_p)
        {
            ...
            DevCtrl_p->Exceptions_p->ECCDropAction = DeviceEx.ECCDropAction;
        ...
        if (DevCtrl_p->L3L4_p)
        {
            DevCtrl_p->L3L4_p->flags = L3L4.flags;
        }
#endif
    }
    ...
    if (CtrlPkt_p)
    {
        ...
        Rc = EIP163_ControlPacket_Read(&CfyE_Devices[DeviceId]->IOArea,
                                                ChannelId,
                                                &DeviceCP);
        if (Rc != EIP163_NO_ERROR)
        {
    ...
    if (HdrPrs_p)
    {
       ...
       if(HdrPrs_p->L4_Ports_p)
       {
        ...
        Rc = EIP163_HeaderParser_Read(&CfyE_Devices[DeviceId]->IOArea,
                                      ChannelId,
                                      &HdrParserControl);
        if (Rc != EIP163_NO_ERROR)
        {
        ...
        if (HdrPrs_p->EgressHeader_p)
        {
            ...
            EHdr_p->EgressHeaderEtype = EgressHeader.EgressHeaderEtype;
            EHdr_p->fEnable = EgressHeader.fEnable;
        }
#endif
        ...
        if (HdrPrs_p->IPsec_Parser_p)
        {
            CfyE_IPSEC_Parser_t * IPsec_p = HdrPrs_p->IPsec_Parser_p;

            IPsec_p->fParseIP           = IPsec_Parser.fParseIP;
            IPsec_p->fParseUDP          = IPsec_Parser.fParseUDP;
            IPsec_p->fParseNAT          = IPsec_Parser.fParseNAT;
            IPsec_p->fParseESP          = IPsec_Parser.fParseESP;
            IPsec_p->fParseIKE          = IPsec_Parser.fParseIKE;
            IPsec_p->fParseNATIKE       = IPsec_Parser.fParseNATIKE;
            IPsec_p->fParseNATKeepAlive = IPsec_Parser.fParseNATKeepAlive;
            IPsec_p->fVerifyUDPChkSum   = IPsec_Parser.fVerifyUDPChkSum;
            IPsec_p->fMACDACheck        = IPsec_Parser.fMACDACheck;
            IPsec_p->fAllowFragments    = IPsec_Parser.fAllowFragments;
            IPsec_p->fIgnoreIPv4ChkSum  = IPsec_Parser.fIgnoreIPv4ChkSum;
            IPsec_p->fSubParseIKE       = IPsec_Parser.fSubParseIKE;
            IPsec_p->fSubParseNATIKE    = IPsec_Parser.fSubParseNATIKE;
            IPsec_p->fSubParseNATKeepAlive = IPsec_Parser.fSubParseNATKeepAlive;

            IPsec_p->IKE_Port           = IPsec_Parser.IKE_Port;
            IPsec_p->NAT_Port           = IPsec_Parser.NAT_Port;

            memcpy(IPsec_p->MAC_DA,
                   IPsec_Parser.MAC_DA,
                   sizeof(IPsec_Parser.MAC_DA));
        }
#endif
        ...
        if (HdrPrs_p->MPLS_Parser_p)
        {
            ...
            MPLS_p->MPLS3_Select1 = MPLS_Parser.MPLS3_Select1;
            MPLS_p->MPLS3_Select2 = MPLS_Parser.MPLS3_Select2;
            MPLS_p->MPLS4_Select1 = MPLS_Parser.MPLS4_Select1;
            MPLS_p->MPLS4_Select2 = MPLS_Parser.MPLS4_Select2;
            MPLS_p->MPLS5_Select1 = MPLS_Parser.MPLS5_Select1;
            MPLS_p->MPLS5_Select2 = MPLS_Parser.MPLS5_Select2;
       ...
       {
#ifdef ADAPTER_EIP163_VXLAN_ENABLE
            HdrPrs_p->L4_Ports_p->VxLAN_Port1 = L4_Ports.VxLAN_Port1;
            HdrPrs_p->L4_Ports_p->VxLAN_Port2 = L4_Ports.VxLAN_Port2;
#endif
       }
#endif
    }
    ...
    if (StatControl_p)
    {
        ...
        Rc = EIP163_StatisticsControl_Read(&CfyE_Devices[DeviceId]->IOArea,
                                           &StatControl);
        if (Rc != EIP163_NO_ERROR)
        {
    ...
    if (EOPConf_p)
    {
        ...
        Rc = EIP163_Device_EOP_Configuration_Get(&CfyE_Devices[DeviceId]->IOArea,
    ...
    if (ECCConf_p)
    {
        ...
        Rc = EIP163_Device_ECC_Configuration_Get(&CfyE_Devices[DeviceId]->IOArea,
        ...
        CfyE_DeviceStatus_t * const DeviceStatus_p)
{
    ...
    if (DeviceStatus_p->ECCStatus_p)
    {
        ...
        Rc = EIP163_Device_ECCStatus_Get(&CfyE_Devices[DeviceId]->IOArea,
                                         ECCStatus);
        if (Rc != EIP163_NO_ERROR)
        {
    ...
    if (DeviceStatus_p->PktProcessDebug_p)
    {
        ...
        Rc = EIP163_Device_PktProcessDebug_Get(&CfyE_Devices[DeviceId]->IOArea,
                                          &PktProcessDebug);
        if (Rc != EIP163_NO_ERROR)
        {
    ...
    if (DeviceStatus_p->fExternalTCAM_p)
    {
        *DeviceStatus_p->fExternalTCAM_p =
            CfyE_Devices[DeviceId]->fExternalTCAM;
    }
        ...
        const CfyE_Ch_Mask_t * const ChannelMask_p)
{
    ...
    Rc = EIP163_Device_InsertSOP(&CfyE_Devices[DeviceId]->IOArea,
        ...
        const CfyE_Ch_Mask_t * const ChannelMask_p)
{
    ...
    Rc = EIP163_Device_InsertEOP(&CfyE_Devices[DeviceId]->IOArea,
                                 (const EIP163_Ch_Mask_t * const) ChannelMask_p);
    if (Rc != EIP163_NO_ERROR)
    {
        ...
        const bool fBypass)
{
    ...
    Rc = EIP163_Device_Update(&CfyE_Devices[DeviceId]->IOArea,
                              ChannelId,
                              &DeviceControl);
    if (Rc != EIP163_NO_ERROR)
    {
