...
MODULE_PARM_DESC(dump_skb, "Ether Type used for EIP SA Tag insertion/detection");
...
#define eip_dump_skb(skb, fmt, args...) ({ \
	if (dump_skb) { \
		pr_info("DUMP SKB (%u) at %s:%d: " fmt , skb->len, __func__, __LINE__, ## args); \
		__eip_dump_skb(skb); \
	} \
})

static inline void __update_outer_v4(struct sk_buff *skb,
		u8 **outer, u8 **inner, u16 *outer_hdr_len, u16 *esp_len)
{
...
static inline void __update_outer_v6(struct sk_buff *skb,
		u8 **outer, u8 **inner, u16 *outer_hdr_len, u16 *esp_len)
{
	...
	u16 new_outer_plen = _outer_hdr_len - sizeof(struct ipv6hdr) + inner_len;
 ...
 */
static u8 * __fixup_tx_ip(struct sk_buff *skb, u8 proto, u16 *trim_header, u16 *trim_trailer)
{
	...
	if (proto == IPPROTO_IPIP)
		__update_outer_v4(skb, &outer, &inner, &outer_hdr_len, &esp_len);
	else if (proto == IPPROTO_IPV6)
		__update_outer_v6(skb, &outer, &inner, &outer_hdr_len, &esp_len);
	else
		return NULL;
...
static int __eip_ipsec_fixup_tx_skb(struct sk_buff *skb,
		u8 proto, const struct eip_sa_tag *sa_tag)
{
	...
	new_ethhdr = container_of((__be16 *)new_eth_proto, struct eip_ethhdr, proto);
...
static struct xfrm_state *__raw_eip_find_rx_xs(struct eip_ipsec_link *ilink, unsigned int sa_index)
{
	struct eip_xfrm_state *eip_xs;
...
static struct xfrm_state *eip_rx_xs_get(struct eip_ipsec_link *ilink, unsigned int sa_index)
{
